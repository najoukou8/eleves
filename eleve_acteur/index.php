...
$texteintro= "<h2 style='background-color: #ddd; padding : 10px;border-radius: 16px;'>Cette interface capitalise pour les étudiants de génie  industriel les expériences  « 100% acteur de mon école » tout au long de leur scolarité.<br></h2>";
...
$casOn=0;
$texte_table='expériences  100 % acteur de mon école';
$table="eleveacteur";
$cleprimaire='eleveacteur_id';
$autoincrement='eleveacteur_id';
$cleetrangere2='eleveacteur_code_etud';
...
$indexlien2='Code etu';
$ordrecleeetrangere3='';
$cleetrangere3='eleveacteur_code_etud';
...
$champ_date_modif='eleveacteur_date_modif';
$champ_modifpar='eleveacteur_modifpar';
...
$liste_champs_obligatoires=array('eleveacteur_axe','eleveacteur_date_debut','eleveacteur_description','eleveacteur_date_debut','eleveacteur_date_fin','eleveacteur_temps_passe');
$liste_champs_lecture_seule=array('eleveacteur_log');
...
$liste_champs_lecture_seule_ou_invisibles_affectes_en_ajout=array('eleveacteur_log');
$liste_champs_invisibles=array('eleveacteur_id');

$liste_champs_dates=array('eleveacteur_date_debut','eleveacteur_date_fin');
...
$liste_champs_select_plus=array('eleveacteur_statut');
...
$liste_choix_lib_eleveacteur_statut=array('en création','saisie terminée','expérience validée','expérience non validée','à compléter par une autre expérience');
$liste_choix_code_eleveacteur_statut=array(0,1,2,3,4);
...
$liste_champs_tableau=array('Nom','Prénom 1','annee','eleveacteur_statut','eleveacteur_date_debut','eleveacteur_date_fin','annee_scol');
$liste_champs_filtre=array('Nom','annee','eleveacteur_statut','eleveacteur_date_debut');
// pour les filtres si il faut aller plus loin que select distinct
$liste_champs_filtre_partiel=array('eleveacteur_date_debut');
$liste_champs_filtre_partiel_traitement=array('eleveacteur_date_debut'=>" CASE
WHEN month(eleveacteur_date_debut) < 9 THEN concat(year(eleveacteur_date_debut)-1,'-',year(eleveacteur_date_debut))
ELSE
concat(year(eleveacteur_date_debut),'-',year(eleveacteur_date_debut)+1)
END  ");
...
$liste_libelles_tableau=array('Prénom 1'=>'Prénom');
...
$liste_libelles_champ=array('eleveacteur_description'=>'Intitulé de l\'expérience eleve acteur 70 c maxi');
...
$liste_tailles_champ=array('annee_scol'=>10);
...
$liste_champs_tableau_calcule=array('annee_scol');
// pour accéder à la page
$login_autorises=array_merge($ri_modif_groupe,array('administrateur','janym','viardch'));
...
$login_autorises_clone=array('administrateur','janym','viardch');
// pour pouvoir  ajouter
$login_autorises_ajout=array('administrateur','janym','viardch');
// pour pouvoir  supprimer
$login_autorises_suppression=array('administrateur','janym','viardch');
// pour pouvoir  modifier
$login_autorises_modif=array('administrateur','janym','viardch');
// pour pouvoir  accéder à détails : formulaire de modification sans validation
$login_autorises_details=array('administrateur','janym','viardch');
// pour pouvoir  exporter
$login_autorises_export=array('administrateur','janym','viardch');
// email correspondant à administrateur
...
{
// on récupère le login du connecté dans $_SERVER
...
$gestionnaire_mail='Marie-laure.perenon@grenoble-inp.fr';
$libellestatut=array('en création','saisie terminée','expérience validée');
...
// si il existe bien dans la base eleves 
 if ($nombre!=0){
	 // si il  essaie de modifier il faut verifier aussi si c'est bien sa fiche 

	 
		 $query= "SELECT  eleveacteur_code_etud FROM eleveacteur  where eleveacteur_id= :expe_id";
			$preparequery=$connexion->prepare($query);
			$res=$preparequery->execute(array('expe_id' =>$_GET['mod'] ));			
			$u =$preparequery->fetch(PDO::FETCH_OBJ); 
			if($_GET['mod']!='' and $u->eleveacteur_code_etud!=$e->codeEtu)
			{
				echo "problème de concordance d`'identifiants $u->eleveacteur_code_etud différent de $e->codeEtu ";
			// on sort
			exit();
			}	
			else
			{
				...
				$login_autorises_details[]=$loginConnecte; 		
				 ...
				 $codeEtudOK=$e->codeEtu;
					// debug($e->codeEtu);		 
				$filtre_initial="where eleveacteur_code_etud='".$codeEtudOK."' and ";	
			}			
	}
 }
...
$_POST['eleveacteur_log']="Etape -}0 suite à la création de l'enregistrement  effectuée par : ".$nomloginConnecte." le ".date("d.m.y")  ." à ".date("H:i:s")  ."\r\n";
...
$_POST['eleveacteur_statut']=1; 
	$_POST['eleveacteur_log'].="Etape 0 " ."-}".$_POST['eleveacteur_statut'] ." par fin de la saisie par ".$nomloginConnecte ." le ".date("d.m.y")  ." à ".date("H:i:s")  ."\r\n";	
	// message affiché :
$message.="Votre soumission d'expérience 100% acteur de mon école a été enregistrée dans la base et sera traité en vue de sa validation <br>lors de la prochaine séance d'évaluation de ces demandes (nous tentons de suivre cela mensuellement).";	
 // il faudra aussi envoyer un mail
 echo " on envoie un mail aux gestionnaires<br>";
  $messagem.="Une nouvelle experience 100% acteur de mon école a été saisie par  ".$nomloginConnecte ." pour un départ \n
consultation : https://ensgiweb.inpg.fr/eleves/eleveacteur/index.php";
 $messagem .= " \nCordialement\n\n";	

			  // On prepare l'email : on initialise les variables
		$objet = "fin de saisie de expérience eleve-acteur par ".$nomloginConnecte ;
				// On envoi l’email à la gestionnaire 
		   if ($gestionnaire_mail !=''){			   
		envoimail($gestionnaire_mail,$objet,$messagem);
		//envoimailtest($gestionnaire_mail,$objet,$messagem);
		}
				
  }
  else // on est en cours de saisie*
  {
	  $message.=" Votre soumission d'expérience 100% acteur de mon école  a été enregistrée dans la base mais n'est pas encore soumis à l'administration de l'école; <br>Vous pouvez modifier et terminer la saisie pour que l'école le prenne en compte lors de sa prochaine évaluation de ces demandes.<br><br>";
  }
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
...
// pour ne pas stocker du html dans la bdd
 ...
 // on ne fait rien
  ...
  elseif(in_array($ci2,$liste_champs_invisibles)and (!in_array($ci2,$liste_champs_lecture_seule_ou_invisibles_affectes_en_ajout)))
 {
 // on ne fait rien pour récupérer si elle existe la valeur par défaut définie dans la bdd
 }
   elseif(in_array($ci2,$liste_champs_lecture_seule) and (!in_array($ci2,$liste_champs_lecture_seule_ou_invisibles_affectes_en_ajout)))
 {
 // on ne fait rien pour récupérer si elle existe la valeur par défaut définie dans la bdd
 }
  else{
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 if(((in_array($loginConnecte,$login_autorises_modif)or empty($login_autorises_modif)) and !$etudOK) or  ($etudOK and $_POST['eleveacteur_statut'] <=1   )){
	 //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
if($_POST['statutsauv']!= $_POST['eleveacteur_statut'])
{
	$_POST['eleveacteur_log'].="Etape".$_POST['statutsauv'] ."-}".$_POST['eleveacteur_statut'] ." par ".$nomloginConnecte ." le ".date("d.m.y")  ." à ".date("H:i:s")  ."\r\n";		
}	 
// est ce que l'étudiant a ppuyé sur le bouton de fin de saisie
if( $_POST['bouton_finsaisie_mod']!='')
  { 
   // si on a appuyé sur le bouton j'ai terminé on passe statut à 1
$_POST['eleveacteur_statut']=1; 
	$_POST['eleveacteur_log'].="Etape".$_POST['statutsauv'] ."-}".$_POST['eleveacteur_statut'] ." par fin de la saisie par ".$nomloginConnecte ." le ".date("d.m.y")  ." à ".date("H:i:s")  ."\r\n";	
	// message affiché :
$message.="Votre soumission d'expérience 100% élève acteur a été enregistrée dans la base et sera traité en vue de sa validation <br>lors de la prochaine séance d'évaluation de ces demandes (nous tentons de suivre cela mensuellement).";	
 // il faudra aussi envoyer un mail
 echo " on envoie un mail aux gestionnaires<br>";
  $messagem.="Une nouvelle experience expérience 100% élève acteur a été saisie par  ".$nomloginConnecte ." pour un départ \n
consultation : https://ensgiweb.inpg.fr/eleves/eleveacteur/index.php";
 $messagem .= " \nCordialement\n\n";	
...
envoimail($gestionnaire_mail,$objet,$messagem);
  ...
  elseif($_POST['statutsauv']==1 and  $_POST['eleveacteur_statut']==2)
  {
	 // il faudra aussi envoyer un mail
// si on est en mode clone on envoie au connecté
 ...
 $messagem .= "validation de l'expérience \n
Cordialement\n
\n";	

			  // On prepare l'email : on initialise les variables
		$objet = "validation  de expérience eleve-acteur   ".$nomloginConnecte ;
				// On envoi l’email à l'étudiant 
		   if ($_POST['email_aff'] !=''){	
		envoimail($destinatairemail ,$objet,$messagem);
		//envoimailtest($destinatairemail ,$objet,$messagem);
		}  	  
  }
    elseif ($_POST['statutsauv']==1 and  $_POST['eleveacteur_statut']==0)
  {
	 // il faudra aussi envoyer un mail
 echo " on envoie un mail à l'étudiant <br>";
 
 $messagem .= "refus de validation de l'expérience \n
 il va falloir reprendre  votre saisie
Cordialement\n
\n";	

			  // On prepare l'email : on initialise les variables
		$objet = "refus de validation  de expérience eleve-acteur   ".$nomloginConnecte ;
				// On envoi l’email à l'étudiant 
		   if ($_POST['email_aff'] !=''){			   
	envoimail($destinatairemail ,$objet,$messagem);
	//	envoimailtest($destinatairemail ,$objet,$messagem);
...
if($table2=='')
 ...
 $$ci2=mysql_Type_Time($u->$ci2);
  ...
  echo"       <table><tr>  ";
	 ...
	 echo"<input type='hidden' name='statutsauv' value=\"".$eleveacteur_statut."\">"; 
	 // on stocke l'email de l'étudiant
	 $query2="SELECT annuaire.`Mail cano.` as mailetu FROM etudiants
                  left outer join annuaire on upper(etudiants.`Code etu`)=annuaire.`code-etu`
				 WHERE `Code etu` = '". $eleveacteur_code_etud ."';";
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		...
		//	else
		 echo"<input type='hidden' name='".$unchamps."' value=\"".$$unchamps."\">\n";
		   ...
		   elseif (($unchamps == 'eleveacteur_nbre_jours' or $unchamps == 'eleveacteur_statut' 
		   or $unchamps == 'eleveacteur_commentaire')
		   and $etudOK ){	
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	    if ((in_array($loginConnecte,$login_autorises_modif) and !$etudOK ) or ( $etudOK and $eleveacteur_statut <1 ))
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++	
   {
  echo"<input type='Submit' name='bouton_mod' value='modifier'>";
  }
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
if ($eleveacteur_statut == 0 and $etudOK )
...
if ($unchamps != $autoincrement and $unchamps != $champ_date_modif and $unchamps != $champ_modifpar ){
	 ...
	 // si on a une table liée
	  if ($unchamps == $cleetrangere2 ){
		  ...
		  if(!$etudOK )
		  {
	       echo affichemenusqlplusnc($commentaire,$unchamps,$indexlien2,'select * from '.$table2 ." ".$ordrecleeetrangere2,$liste_champs_lies_pour_formulaire_ajout2[0],'',$connexion,$liste_champs_lies_pour_formulaire_ajout2[1],'',' required ');

		  }
		else{
			echo afficheonlysqlplusnc($commentaire,'onlyaffnom',$indexlien2,'select * from '.$table2 ." ".$ordrecleeetrangere2,$liste_champs_lies_pour_formulaire_ajout2[0],$codeEtudOK,$connexion,$liste_champs_lies_pour_formulaire_ajout2[1],'',' required ');
		 echo"<input type='hidden' name='".$unchamps."' value=\"".$codeEtudOK."\">\n";
		}
	  }

	  // elseif ($unchamps == 'eleveacteur_code_etud' and $etudOK==1 ){	
		//	 echo affichechamp($commentaire,$unchamps,$codeEtudOK,'10','1');
		//  }
	   elseif ($unchamps == 'eleveacteur_nbre_jours' and $etudOK ){		   		
		  }
	   elseif ($unchamps == 'eleveacteur_statut' and $etudOK ){
		  }		  
	   elseif ($unchamps == 'eleveacteur_commentaire' and $etudOK ){
		  }		
...
echo "<br>";
 //echo  "<b>Merci de remettre au format papier les documents justificatifs auprès du service des relations internationales de GI</b>";
  echo " </form>";
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++	
    echo    "<form id='annulation' method=post action=$URL> ";
	 	  //on passe tous les arg reçus en get  en hidden sauf mod et add
...
echo"<center> <h2> ";
echo " pas d'enregistrement trouvé dans ".$texte_table ."</H2>";
}
...
while ($u = $req->fetch(PDO::FETCH_OBJ)) {	
	...
	$req2 = $connexion->query("SELECT * FROM eleveacteurdocuments  where  	doc_idEleveacteur =".$u->eleveacteur_id." ");
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     echo "<A href=". $URL."?mod=".$$cleprimaire."&".$filtrerech." >Mod</A>";
	 	echo" </td><td>";	  
     echo "<A href=documenteleveacteur.php?offre=".$$cleprimaire."&".$filtrerech." >Docs(".$nombreDocs. ").</A>";	
	 }
