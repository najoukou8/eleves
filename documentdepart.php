...
if ( $_GET['offre']!='') 
{
	...
	// si il existe bien dans la base eleves 
	 if ($nombre!=0){
		 		 // il faut encore vérifier que son code etu  est bien celui présent dans la fiche du depart
		$query= "SELECT  code_etudiant FROM departs  where code_depart= :offre";
			...
			{echo "problème de concordance d`'identifiants $u->code_etudiant différent de $e->codeEtu ";
			}
		 }
	 }	
...
if(!isset($erreur)) //S'il n'y a pas d'erreur, on upload
{
    ...
    if(move_uploaded_file($_FILES['docfil']['tmp_name'], $chemin_local_depart . $fichier)) //Si la fonction renvoie TRUE, c'est que ça a fonctionné...
     {
...
if(in_array($loginConnecte,$login_autorises_suppression) or empty($login_autorises_suppression)){
 ...
 echo "<a href='".$chemin_depart.$doc_lienDoc ."' target=\"_blank\"> Télécharger</A>";
	        echo"</td> </tr>";
	   }
	   //pdo	   
	$req->closeCursor();
