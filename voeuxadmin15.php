...
<head>
<title>gestion des voeux 15</title>
...
// les groupes sont retrouvés avec $racineGroupes.'-01' ......
$query3="select substr(groupes.libelle,LENGTH('".$racineGroupes15."')+2,2) as gpegip, code_etudiant from ligne_groupe
left outer join groupes on groupes.code=ligne_groupe.code_groupe
 where groupes.libelle like '".$racineGroupes15."-%' ";
...
{
echo 'erreur pas de groupe correspondant dans la base élèves';
}
...
$where="where etudiants_scol.annee like '".$gpecible15."%' ";
//$nom_univ='';
...
if(in_array($login,$voeux_liste15)){
...
//test si admin ent ou si on vient de creation de stage
 if(1){
 // on efface 
   $query = "delete from giptabmalus  where TabMalusIdEleve='".$_POST['codeEtudiant']."' and TabMalusIdCampagne ='".$idsondage15."'";
 ...
 // on cree
  $query = "insert into giptabmalus  (TabMalusIdEleve,TabMalusIdCampagne,TabMalusNote,TabMalusDateModif,TabMalusModifpar)values (".$_POST['codeEtudiant'].",'".$idsondage15."',".$_POST['TabMalusNote'].",now(),'".$login."')";
//echo $query;
   $result = mysql_query($query,$connexion);
   if($result){
   $message = "Fiche <b>";
...
#$sql2="'$gipIdEvaluateur[$i]','$gipIdEvalue[$i]',$gipNote1[$i],$gipNote2[$i],$gipNote3[$i],$gipNote4[$i],'$gipCommentaire[$i]',now()";
    $query = "UPDATE $table SET $sql1";
	   $query .= " WHERE gipIdEvaluateur=".$_POST['gipIdEvaluateur']." and gipIdEvalue=".$_POST['gipIdEvalue'] ." and gipIdCampagne='".$idsondage15."'";
...
// et on les récupère
$query= "SELECT $table.*  FROM $table where gipIdEvaluateur='".$_GET['mod'] ."'and gipIdCampagne='".$idsondage15."'";
//echo $query ;
$result=mysql_query($query,$connexion);
...
$date_modif_prec=mysql_Time($y->gipDateModif);
 ...
 echo "<center><h1>$titrevoeux15<br>";
...
echo    "<form name='tableaunote' method=post action=$URL> ";
  ...
  echo "<table>";
	...
	echo affichechamp('Date modification','affdatemodif',$date_modif_prec,'',1);
	  echo affichechamp('Date limite ','affdatelimite',$datelimitevoeux15,'10',1);
...
foreach ($criteresGip15 as $criterelib)
{
...
echo "<td>".$etudiants_nom[$codeEtu] ." " .$etudiants_prenom[$codeEtu]."</td>";
	...
	//echo"<input type='hidden' name='critere[]' value=\"".$criterelib."\">";		
		echo afficheonly ('',$appreciationsLibGip15[array_search($gipNote1[$codeEtu],$appreciationsNotesGip15)],'font',"font color='".$appreciationsNotesCouleursGip15[array_search($gipNote1[$codeEtu],$appreciationsNotesGip15)]."'");
		echo afficheonly ('',$appreciationsLibGip15[array_search($gipNote2[$codeEtu],$appreciationsNotesGip15)],'font',"font color='".$appreciationsNotesCouleursGip15[array_search($gipNote2[$codeEtu],$appreciationsNotesGip15)]."'");
		echo afficheonly ('',$appreciationsLibGip15[array_search($gipNote3[$codeEtu],$appreciationsNotesGip15)],'font',"font color='".$appreciationsNotesCouleursGip15[array_search($gipNote3[$codeEtu],$appreciationsNotesGip15)]."'");		
		echo afficheonly ('',$appreciationsLibGip15[array_search($gipNote4[$codeEtu],$appreciationsNotesGip15)],'font',"font color='".$appreciationsNotesCouleursGip15[array_search($gipNote4[$codeEtu],$appreciationsNotesGip15)]."'");		
...
echo "<th>Commentaire<br>validé</th>";
  //  on vérifie s'il y a déjà une fiche de voeu pour cet élève 
// et on les récupère
$query= "SELECT $table.*  FROM $table where gipIdEvalue='".$_GET['mod_evalue'] ."' and gipIdCampagne='".$idsondage15."'";
//echo $query ;
$result=mysql_query($query,$connexion);
while($y=mysql_fetch_object($result)){
...
echo "<td>".$etudiants_nom[$y->gipIdEvaluateur] ." " .$etudiants_prenom[$y->gipIdEvaluateur]."</td>";
	...
	//echo"<input type='hidden' name='gipIdEvaluateur[]' value=\"".$_GET['mod']."\">";	
	//echo"<input type='hidden' name='critere[]' value=\"".$criterelib."\">";		
		echo afficheonly ('',$appreciationsLibGip15[array_search($y->gipNote1,$appreciationsNotesGip15)],'font',"font color='".$appreciationsNotesCouleursGip15[array_search($y->gipNote1,$appreciationsNotesGip15)]."'");
		echo afficheonly ('',$appreciationsLibGip15[array_search($y->gipNote2,$appreciationsNotesGip15)],'font',"font color='".$appreciationsNotesCouleursGip15[array_search($y->gipNote2,$appreciationsNotesGip15)]."'");
		echo afficheonly ('',$appreciationsLibGip15[array_search($y->gipNote3,$appreciationsNotesGip15)],'font',"font color='".$appreciationsNotesCouleursGip15[array_search($y->gipNote3,$appreciationsNotesGip15)]."'");		
		echo afficheonly ('',$appreciationsLibGip15[array_search($y->gipNote4,$appreciationsNotesGip15)],'font',"font color='".$appreciationsNotesCouleursGip15[array_search($y->gipNote4,$appreciationsNotesGip15)]."'");		
...
echo    "</form > ";
 ...
 //on fait une boucle pour créer les variables issues de la table stage
	...
	$sqlquery2="SELECT * FROM giptabmalus where TabMalusIdEleve =  '".$_GET['add']."' and TabMalusIdCampagne='".$idsondage15."'";
	//echo $sqlquery2;
	$resultat2=mysql_query($sqlquery2,$connexion );
  ...
  echo afficheonly("","Bonus Malus",'b' ,'h3');
	...
	{	 
	 ...
	 echo "</tr><tr>";
echo affichemenu('Vous pouvez changer le Bonus/malus','TabMalusNote',$bonusMalusNotesGip15,$etudiants_malus);
...
// et on les récupère
$query= "SELECT $table.*  FROM $table where gipCommentaire !='' and  gipCommentaire is not null and gipIdCampagne='".$idsondage15."'";
//echo $query ;
$result=mysql_query($query,$connexion);
if (mysql_num_rows($result) >0)
...
echo "<td>".$etudiants_nom[$tabValidCommentaires['evalue']] ." " .$etudiants_prenom[$tabValidCommentaires['evalue']]."</td>";
	...
	//echo"<input type='hidden' name='critere[]' value=\"".$criterelib."\">";		
		echo afficheonly ('',$appreciationsLibGip15[array_search($tabValidCommentaires['note1'],$appreciationsNotesGip15 )],'font',"font color='".$appreciationsNotesCouleursGip15[array_search($tabValidCommentaires['note1'],$appreciationsNotesGip15 )]."'");
		echo afficheonly ('',$appreciationsLibGip15[array_search($tabValidCommentaires['note2'],$appreciationsNotesGip15 )],'font',"font color='".$appreciationsNotesCouleursGip15[array_search($tabValidCommentaires['note2'],$appreciationsNotesGip15 )]."'");
		echo afficheonly ('',$appreciationsLibGip15[array_search($tabValidCommentaires['note3'],$appreciationsNotesGip15 )],'font',"font color='".$appreciationsNotesCouleursGip15[array_search($tabValidCommentaires['note3'],$appreciationsNotesGip15 )]."'");		
		echo afficheonly ('',$appreciationsLibGip15[array_search($tabValidCommentaires['note4'],$appreciationsNotesGip15 )],'font',"font color='".$appreciationsNotesCouleursGip15[array_search($tabValidCommentaires['note4'],$appreciationsNotesGip15 )]."'");		
...
// echo affichealerte('Patientez quelques instants après avoir cliqué sur Modifier');
  echo "<center><a href=$URL>Revenir à l'accueil de l'évaluation</a>";
	  ...
	  else
	  {
	   ...
	   echo "<a href=voeuxadmin15.php> Revenir</a>";
	  }
...
if ($affichetout)  {
 ...
 echo" <table width=100% height=100%><tr><td><center>  ";
  ...
  // $query.=$where."  ".$orderby;
    $query="SELECT  etudiants.Nom ,etudiants.`Prénom 1` ,etudiants.`Code etu` as codetu,etudiants.`Lib dac` as provenance , annuaire.`Mail effectif`  , departements.dep_libelle,  substr(groupes.libelle,LENGTH('".$racineGroupes15."')+2,2) as gpegip,etudiants_scol.annee ,giptabmalus.*
FROM ligne_groupe
...
LEFT OUTER JOIN giptabmalus ON concat(etudiants.`Code etu`,'".$idsondage15."')  = concat(giptabmalus.TabMalusIdEleve,giptabmalus.TabMalusIdCampagne)
 where groupes.libelle like '".$racineGroupes15."-%' ";
...
echo"<th>Prénom</th>";
echo afficheentete($racineGroupes15,'gpegip',$_GET['env_orderby'],$_GET['env_inverse'],$filtre,$URL);
...
foreach($criteresGipAbreges15 as $critAbr)
{
 ...
 foreach($criteresGipAbreges15 as $critAbr)
{
  ...
  $csv_output .="commentaire".";"."valid commentaire".";"."date modif".";";
...
$query2= "SELECT $table.*  FROM $table where gipIdEvaluateur='".$codeetu ."' and gipIdCampagne='".$idsondage15."'";
...
foreach($champs as $ci2){
 ...
 // on récupère toutes les notes RECUES par cet eleve
			   ...
			   $tot3=0;
			   $tot4=0  ;
		  $query3= "SELECT $table.*  FROM $table where gipIdEvalue='".$codeetu ."' and gipIdCampagne='".$idsondage15."'";
$result3 = mysql_query($query3,$connexion ); 
$nombre3= mysql_num_rows($result3);
		  ...
		  if($nombre3!=0)
		  {
				 ...
				 $moy1=round($tot1/$nombre3,2);
					foreach($appreciationsMoyCouleursGip15 as $appreciationsCouleur)
						{
						//echo "if($moygenerale<= $seuilsMoyennesGip[$i+1] && $moygenerale > $seuilsMoyennesGip[$i])";
								if($moy1<= $seuilsMoyennesGip15[$i+1] && $moy1 > $seuilsMoyennesGip15[$i]){								
				...
				echo $moy1;
				 ...
				 $moy2=round($tot2/$nombre3,2);
					foreach($appreciationsMoyCouleursGip15 as $appreciationsCouleur)
						{
						//echo "if($moygenerale<= $seuilsMoyennesGip15[$i+1] && $moygenerale > $seuilsMoyennesGip15[$i])";
								if($moy2<= $seuilsMoyennesGip15[$i+1] && $moy2 > $seuilsMoyennesGip15[$i]){								
									$couleurCellule="bgcolor='".$appreciationsCouleur."'";
									break;
								}
						$i++;
				...
				echo $moy2;
				 ...
				 $moy3=round($tot3/$nombre3,2);;
					foreach($appreciationsMoyCouleursGip15 as $appreciationsCouleur)
						{
						//echo "if($moygenerale<= $seuilsMoyennesGip15[$i+1] && $moygenerale > $seuilsMoyennesGip15[$i])";
								if($moy3<= $seuilsMoyennesGip15[$i+1] && $moy3 > $seuilsMoyennesGip15[$i]){								
									$couleurCellule="bgcolor='".$appreciationsCouleur."'";
									break;
								}
						$i++;
				...
				echo $moy3;
				 ...
				 $moy4=round($tot4/$nombre3,2);;
					foreach($appreciationsMoyCouleursGip15 as $appreciationsCouleur)
						{
						//echo "if($moygenerale<= $seuilsMoyennesGip15[$i+1] && $moygenerale > $seuilsMoyennesGip15[$i])";
								if($moy4<= $seuilsMoyennesGip15[$i+1] && $moy4 > $seuilsMoyennesGip15[$i]){								
									$couleurCellule="bgcolor='".$appreciationsCouleur."'";
									break;
								}
						$i++;
				...
				echo $moy4;
				 ...
				 $moygenerale=round(($tot1/$nombre3+$tot2/$nombre3+$tot3/$nombre3+$tot4/$nombre3)/4,2);
					// if($moygenerale>=1.5){
						// $couleurCellule="bgcolor='".$appreciationsMoyCouleursGip15[4]."'";
					// }
					// if($moygenerale<1.5 && $moygenerale>=1){
						// $couleurCellule="bgcolor='".$appreciationsMoyCouleursGip15[3]."'";
					// }
					// if($moygenerale<1 && $moygenerale>0.5){
						// $couleurCellule="bgcolor='".$appreciationsMoyCouleursGip15[2]."'";
					// }
					// if($moygenerale <= 0.5 && $moygenerale>=-0.5){
						// $couleurCellule="bgcolor='".$appreciationsMoyCouleursGip15[1]."'";
					// }
					// if($moygenerale<= -0.5){
						// $couleurCellule="bgcolor='".$appreciationsMoyCouleursGip15[0]."'";
					// }
					foreach($appreciationsMoyCouleursGip15 as $appreciationsCouleur)
						{
						//echo "if($moygenerale<= $seuilsMoyennesGip15[$i+1] && $moygenerale > $seuilsMoyennesGip15[$i])";
								if($moygenerale<= $seuilsMoyennesGip15[$i+1] && $moygenerale > $seuilsMoyennesGip15[$i]){								
									$couleurCellule="bgcolor='".$appreciationsCouleur."'";
									break;
								}
						$i++;
