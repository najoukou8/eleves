...
$casOn=1;
$texte_table='entreprises - apprentissage';
$table="entreprises";
$cleprimaire='code';
$autoincrement='code';
$cleetrangere2='';
...
$liste_champs_invisibles=array('entreprise_apprent');
$liste_champs_dates=array();
$liste_champs_heures=array();
$liste_champs_tableau=array('nom','ville');
...
$liste_libelles_champ=array('dirigeant'=>'contact');
...
$liste_ordre_champ=array('dirigeant'=>7);
...
$login_autorises=array('anceychr','rogesj','administrateur');
...
$login_autorises_ajout=array('anceychr','rogesj','patouilm','administrateur');
// pour pouvoir  supprimer
$login_autorises_suppression=array('anceychr','rogesj','administrateur');
// pour pouvoir  modifier
$login_autorises_modif=array('anceychr','rogesj','administrateur','patouilm');
// pour pouvoir  accéder à détails : formulaire de modification sans validation
$login_autorises_details=array('anceychr','rogesj','administrateur');
// pour pouvoir  exporter
$login_autorises_export=array('anceychr','rogesj');
...
echo "		<script src='jsSimpleDatePickrInit.js'></script>";
echo "		<script src='jsSimpleDatePickr.js'></script>";
echo "		<script src='verifheure.js'></script>";
echo "</HEAD><BODY>" ;
...
foreach($champs as $unchamps){	
			 ...
			 // il n'est pas spécifié dans les commentaires des champs dans la  bdd ? on  prend celui spécifié dans $liste_tailles_champ sinon on prend celle de la bdd
...
if ($unchamps != $autoincrement and $unchamps != $champ_date_modif and $unchamps != $champ_modifpar ){
	 ...
	 // si on a une table liée
	  if ($unchamps == $cleetrangere2 ){
	       echo affichemenusqlplus($commentaire,$unchamps,$indexlien2,'select * from '.$table2 ." ".$ordrecleeetrangere2,$liste_champs_lies_pour_formulaire_ajout2[0],'',$connexion,$liste_champs_lies_pour_formulaire_ajout2[1]);

	  }
		  elseif ($unchamps == $cleetrangere3 ){
			   echo affichemenusqlplus($commentaire,$unchamps,$indexlien3,'select * from '.$table3 ." ".$ordrecleeetrangere3,$liste_champs_lies_pour_formulaire_ajout3[0],'',$connexion,$liste_champs_lies_pour_formulaire_ajout3[1]);
				  ...
				  elseif (in_array($unchamps,$liste_champs_lies3) ){
					...
					//]]>
						 ...
						 else
						 {
							  if ($tailleAffichageChamp<$tailleLimiteChampCourt)
							 {
						 echo affichechamp($commentaire,$unchamps,'',$tailleAffichageChamp,'','','','','',$required . ' ' .$placeholder);
							 }
							else{
						 echo "<td><label for=\"".$unchamps."\">".$commentaire."<br>.</label><textarea row = \"4\" cols=\"70\" name=$unchamps id=$unchamps $required></textarea></td>";			
							}	
						 }
}
    echo "</tr><tr>";	 
...
//2018
$where.= "  and entreprise_apprent ='oui' ";
