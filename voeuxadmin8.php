...
$where="where etudiants_scol.annee like '".$gpecible8."%' ";
//$nom_univ='';
...
if(in_array($login,$voeux_liste8)){
...
foreach ($courspossible8 as $coursenum ){
...
elseif($_GET['del']!='') {
 if(in_array($login,$scol_user_liste)){
   ...
   echo "<center><b>seul le service habilité peut effectuer cette operation</b><br>";
      echo "aucune modification effectuée<br></center>";
}//fin du else $login == 
}
//--------------------------------- Modif de la fiche
...
$v=mysql_fetch_object($result) ;
 ...
 //on fait une boucle pour remettre en hiden tous les champs  de la table stage
  ...
  //echo"<input type='hidden' name='mod' value=1>";
	...
	echo "<center><h1>$titrevoeux8</h1><br>";
...
FROM voeux_eleves
WHERE v_id_sondage='".$idsondage8."' and v_num_etudiant='".$codeetu."'
";
