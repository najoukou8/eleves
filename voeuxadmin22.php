...
<head>
<title>gestion des voeux 22</title>
...
$where="where etudiants_scol.annee like '".$gpecible22."%' ";
//$nom_univ='';
...
if(in_array($login,$voeux_liste22)){
...
foreach ($courspossible22 as $coursenum ){
...
$v=mysql_fetch_object($result) ;
 ...
 //on fait une boucle pour remettre en hiden tous les champs  de la table stage
  ...
  //echo"<input type='hidden' name='mod' value=1>";
	...
	echo "<center><h1>$titrevoeux22</h1><br>";
...
if (isset($choix22_1))
	if(!empty($choix22_1))
		echo affichemenunc('1er choix','v_voeux1',$choix22_1,$v_voeux1);
	else
		 echo affichechamp('1er choix','v_voeux1',$v_voeux1);
if (isset($choix22_2))
	if(!empty($choix22_2))
			echo affichemenunc('2eme choix','v_voeux2',$choix22_2,$v_voeux2);
	else
		 echo affichechamp('2eme choix','v_voeux2',$v_voeux2);
	 		  	 echo "</tr><tr>";
if ($v_voeux5!='')
{
			echo "<td><a href='".$chemin_upload22.$v_voeux5 ."'>"."récupérer le document téléchargé : "."</a>$v_voeux5</td>";
...
echo "</tr><tr>";
		  ...
		  echo "<td>Commentaire</td>";
		  	  		 echo "</tr><tr>";
					 if ($v_commentaire==''){
						echo "<td colspan=3><textarea   name='v_commentaire' rows=6 cols=60 placeholder=\"".$question_commentaire22_1."\"></textarea></td> ";	
						}
						else
						{
...
if (isset($choix22_1))echo"<th>voeu1</th>";
if (isset($choix22_2))echo"<th>voeu2</th>";
...
FROM voeux_eleves
WHERE v_id_sondage='".$idsondage22."' and v_num_etudiant='".$codeetu."'
";
   ...
   $csv_output .= "\n";
    ...
    if($login=='administrateur'){ 
     ...
     echo "<A href=". $URL."?mod=$v_id>détails</A>";
	  	 ...
	  	 if($v_voeux5 !=''){
	  echo "<a href='".$chemin_upload22.$v_voeux5 ."'>"."-doc téléchargé"."</a>"; 
		 }		 
