...
{      
  ...
  $error = 'votre fichier est trop gros :  '.(round($_SERVER['CONTENT_LENGTH']/1024/1024)).'Moctets'.
		   ...
		   echo "<center><H2>". $error ."</H2>";
			echo"<br><a href=index.php>revenir à l'accueil</a><br></center>";
		   //exit();
}
...
// pour accéder à la page
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
  // les personnels autorisés
  $interculture_users=array('administrateur','dehemchn','jouffral','gaujalg','lemaireh','malandrs','papav','reverdth','lemairpi');
  
   //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
$login_autorises=$interculture_users;
...
$login_autorises_clone=$interculture_users;
// pour pouvoir  ajouter
$login_autorises_ajout=$interculture_users;
// pour pouvoir  supprimer
$login_autorises_suppression=$interculture_users;
// pour pouvoir  modifier
$login_autorises_modif=$interculture_users;
// pour pouvoir  accéder à détails : formulaire de modification sans validation
$login_autorises_details=$interculture_users;
// pour pouvoir  exporter
$login_autorises_export=$interculture_users;
...
{
	   //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	$filtre_initial=" where doc_idInterculture = '".$_GET['offre']."' and " ;
	...
	// si il existe bien dans la base eleves 
	 if ($nombre!=0){
		 		 // il faut encore vérifier que son login est bien celui présent dans la fiche de l'experience
		$query= "SELECT  interculture_code_etud FROM interculture  where interculture_id= :offre";
			$preparequery=$connexion->prepare($query);
			$res=$preparequery->execute(array('offre' =>$_GET['offre'] ));			
			$u =$preparequery->fetch(PDO::FETCH_OBJ); 
			if($u->interculture_code_etud == $e->codeEtu)
			{
			...
			{echo "problème de concordance d`'identifiants $u->interculture_code_etud différent de $e->codeEtu ";
			}
		 }
	 }	
...
{
	// si un étudiant  arrive ici  , c'est qu'il  a bidouillé les param , donc on ne l'autorise pas à aller plus loin
	// il n' y a rien à faire car il n'a pas été ajouté à $login_autorises
 //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
...
if(!isset($erreur)) //S'il n'y a pas d'erreur, on upload
{
    ...
    if(move_uploaded_file($_FILES['docfil']['tmp_name'], $chemin_local_interculture. $fichier)) //Si la fonction renvoie TRUE, c'est que ça a fonctionné...
     {
 ...
 $$ci2=mysql_Type_Time($u->$ci2);
  ...
  echo"       <table><tr>  ";
    ...
    //+++++++++++++++++++++++++++on n'affiche pas doc_idVoeu_eleves	+++++++++++++++++++++++++++++++++++++++++++++++++++++++
		 elseif ($unchamps== 'doc_idInterculture' )
			 {
				  echo"<input type='hidden' name='".$unchamps."' value=\"".$$unchamps."\">";
			 }
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++		
...
if(in_array($loginConnecte,$login_autorises_suppression) or empty($login_autorises_suppression)){
 ...
 echo "<a href='".$chemin_interculture.$doc_lienDoc ."' target=\"_blank\"> Télécharger</A>";
	        echo"</td> </tr>";
	   }
	   //pdo	   
	$req->closeCursor();
