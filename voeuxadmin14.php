...
//on remplit 1tableau avec les libelles-code cours
 $sqlquery2="SELECT * FROM `".$table_cours14."` order by CODE";
 //echo  $sqlquery2;
$resultat2=mysql_query($sqlquery2,$connexion ); 
while ($v=mysql_fetch_array($resultat2)){
$courscode[]=$v["CODE"] ;
...
$query = "SELECT * FROM voeux_s5_ues where  voeuxS5idCampagne='".$idsondage14."' order by  conflit_edt_ue  ";
...
if(in_array($login,$voeux_liste14)){
...
elseif($_GET['del']!='') {
 ...
 // il faut aussi supprimer les lignes de voeux pour cet etudiant
  $query = "DELETE FROM ligne_voeux_ues5"
      ." WHERE ligvs5_login='".$_GET['login']."' and ligvs5_code_idsondage ='".$idsondage14."' ";
   $result = mysql_query($query,$connexion);
   if($result){
...
// if ($ci2!="rep_voeu_parcours1"  and $ci2!="rep_voeu_parcours2"  and $ci2!="rep_voeu_parcours3" and $ci2!="rep_voeu_parcours4" and $ci2!="rep_voeu_parcours5" and $ci2!="rep_voeu_parcours14" and $ci2!="rep_voeu_inter1"  and $ci2!="rep_voeu_inter2" and $ci2!="rep_voeu_inter3" and $ci2!="rep_voeu_inter4" and $ci2!="rep_voeu_inter5"    ){
 //on ne fait rien
 //}
if ($ci2=="v_date_modif"){
 $sql1.= $ci2."=now(),";}
 ...
 //on fait une boucle pour remettre en hiden tous les champs  de la table stage
  ...
  echo"       <table><tr>  ";
	 ...
	 echo "</tr><tr>";
		  ...
		  echo affichechamp('Date limite ','affdatelimite',$datelimitevoeux14,'10',1);
...
echo "</tr><tr>";
 ...
 echo "</table>";
	 ...
	 $query2 = "SELECT distinct `".$table_cours14."`.*,ligne_voeux_ues5.* FROM `".$table_cours14."` 
	 left outer join ligne_voeux_ues5 on ligne_voeux_ues5.ligvs5_code_ue=`".$table_cours14."`.code 
	 where (ligne_voeux_ues5.ligvs5_login = '".$_GET['login']."'and ligvs5_code_idsondage='".$idsondage14."' ) 
	...
	//{
	echo "<table border=1>";
	echo "<th colspan=5 align=center>Liste des UEs </th>";
	echo "</tr><tr>";
...
foreach($uetotal_s5 as $uecode)
	...
	$coursjetons[$uecode]='';
				...
				//echo affichemenu('Jetons',$temp,$tab_choix_jetons14,$coursjetons[$uecode]);
				$indice=array_search($coursjetons[$uecode], $tab_choix_jetons14);
		  echo affichechamp("",$temp,$libelleJetons14[$indice],3);
				echo " </td> ";
				echo "</tr>";		 
			$i++;
...
WHERE libelle = '".$gpecible14."' and (v_id_sondage='".$idsondage14."'  )
";  
      $query.="  ".$orderby;
 //echo $query;
   $result = mysql_query($query,$connexion ); 
...
echo "<a href=ues_voeux_s5.php?voeuxS5idCampagne=".$idsondage14."> Modifier la liste des UEs<br><br></a>";
...
//on initialise  $csv_output
 ...
 {
  $csv_output .=   $courslibel[$une_ue] ." - ".$une_ue.";";
...
/* 				$csv_output .= nettoiecsv($rep_voeu_parcours1); */
   ...
   // pour l'export excel on recupere tous ses voeux
   	 ...
   	 $query2 = "SELECT distinct `".$table_cours14."`.*,ligne_voeux_ues5.* FROM `".$table_cours14."` 
	 left outer join ligne_voeux_ues5 on ligne_voeux_ues5.ligvs5_code_ue=`".$table_cours14."`.code 
	 where (ligne_voeux_ues5.ligvs5_login = '".$universite->Uid."' and ligvs5_code_idsondage='".$idsondage14."' ) 
	 order by LIBELLE_COURT " ;
	        $result2 = mysql_query($query2,$connexion ); 
 ...
 if (array_key_exists($une_ue,$uechoisies))
		 {		 
		 $indice=array_search($uechoisies[$une_ue], $tab_choix_jetons14);
		  $csv_output .=$libelleJetons14[$indice].";";
		 }
		 else
		 $csv_output .="0;";
 }
...
FROM `voeux_eleves`
WHERE `v_id_sondage`='".$idsondage14."'  )
AND libelle  LIKE '".$gpecible14."'
 ORDER BY Nom";
//echo $query;
 $result = mysql_query($query,$connexion ); 
$nombre= mysql_num_rows($result);
