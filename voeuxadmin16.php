...
$texte_explic_groupe_sie='groupe_sie'.$numero_campagne;
// correction 2019 on récupère le nom du groupe cible SIE dans la bdd et plus dans paramcommuns
//$groupe_sie=$$texte_explic_groupe_sie;
$groupe_sie=$gpecible16;
...
//on remplit 1tableau avec les libelles-code cours
 $sqlquery2="SELECT * FROM `".$table_cours16."` order by CODE";
$resultat2=mysql_query($sqlquery2,$connexion ); 
while ($v=mysql_fetch_array($resultat2)){
$courscode[]=$v["CODE"] ;
...
$query = "SELECT * FROM voeux_s5_ues where  voeuxS5idCampagne='".$idsondage16."' order by  conflit_edt_ue ";
...
while($u=mysql_fetch_object($result)) {
	switch ($u->type_ue){
		...
		break;
		case "transversale sie":
		$uecommunsie[]=$u->code_ue;		
		break;		
		case "idp":
		$ueidp[]=$u->code_ue;
		break;
...
elseif(($_GET['filiere_test']) == 'sie')
{
$sie='oui';
$uecommun=$uecommunsie;
}
...
if(in_array($login,$voeux_liste16)){
...
elseif($_GET['del']!='') {
 ...
 // il faut aussi supprimer les lignes de voeux pour cet etudiant
  $query = "DELETE FROM ligne_voeux_ues5"
      ." WHERE ligvs5_login='".$_GET['login']."' and ligvs5_code_idsondage ='".$idsondage16."' ";
   $result = mysql_query($query,$connexion);
   if($result){
...
// if ($ci2!="rep_voeu_parcours1"  and $ci2!="rep_voeu_parcours2"  and $ci2!="rep_voeu_parcours3" and $ci2!="rep_voeu_parcours4" and $ci2!="rep_voeu_parcours5" and $ci2!="rep_voeu_parcours16" and $ci2!="rep_voeu_inter1"  and $ci2!="rep_voeu_inter2" and $ci2!="rep_voeu_inter3" and $ci2!="rep_voeu_inter4" and $ci2!="rep_voeu_inter5"    ){
 //on ne fait rien
 //}
if ($ci2=="v_date_modif"){
 $sql1.= $ci2."=now(),";}
 ...
 //on fait une boucle pour remettre en hiden tous les champs  de la table stage
  ...
  echo"       <table><tr>  ";
	 ...
	 echo "</tr><tr>";
		  ...
		  echo affichechamp('Date limite ','affdatelimite',$datelimitevoeux16,'10',1);
...
$uecommun=$uecommunipid;
  ...
  // on verifie si'il fait partie du gpe sie
 $query3="select groupes.libelle as libellegpe, groupes.login_proprietaire as propgpe from ligne_groupe
left outer join groupes on groupes.code=ligne_groupe.code_groupe
 where ligne_groupe.code_etudiant='".$_GET['codetu'] ."'";
 $query3.= " and groupes.libelle like '".$groupe_sie."%' ";
 $result3 = mysql_query($query3,$connexion ); 
   if (mysql_num_rows($result3)!=0){
$sie='oui';
echo "<br>filière SIE<br>";
$uecommun=array_merge($uecommun,$ueidp,$ueipid,$uesie);
$uecommun=$uecommunsie;
}
 ...
 $wherecours=" `".$table_cours16."`.CODE like '5%'";
//on remplit 1tableau avec les libelles-code cours
 $sqlquery2="SELECT * FROM `".$table_cours16."` order by CODE";
$resultat2=mysql_query($sqlquery2,$connexion ); 
while ($v=mysql_fetch_array($resultat2)){
...
$coursjetons[$v["CODE"]]='';
	 ...
	 $query2 = "SELECT distinct `".$table_cours16."`.*,ligne_voeux_ues5.* FROM `".$table_cours16."` 
	 left outer join ligne_voeux_ues5 on ligne_voeux_ues5.ligvs5_code_ue=`".$table_cours16."`.code 
	 where (ligne_voeux_ues5.ligvs5_login = '".$_GET['login']."'and ligvs5_code_idsondage='".$idsondage16."' ) 
...
foreach($uetotal_s5 as $uecode)
	...
	$coursjetons[$uecode]='';
		...
		if (	in_array($uecode ,$ueicl))
		{
				...
				echo affichemenu('Jetons',$temp,$tab_choix_jetons,$coursjetons[$uecode]);
				echo " </td> ";
				echo "</tr>";		 
			$i++;
		}	
	...
	{	
	echo "<table border=1>";
	echo "<th colspan=5 align=center>Voeux IDP</th>";
	echo "</tr><tr>";
	echo "<th></th><th>Code cours</th><th>Intitule</th><th>Crédits ECTS</th>";
	echo"<th>  </th>";	
	 $bgcolor='bgcolor=lightgreen';
	...
	$coursjetons[$uecode]='';
		...
		if (	in_array($uecode ,$ueidp))
		{
				...
				echo"   </td><td>" ;
				echo $coursects[$uecode];
				echo"<input type='hidden' name=champ_ue_form_code_idp".$i." value='".$uecode."' >";
				$temp='champ_ue_form_jetons_idp'.$i;
				echo affichemenu('Jetons',$temp,$tab_choix_jetons,$coursjetons[$uecode]);
				echo " </td> ";
				echo "</tr>";		 
			$i++;
	 		     ...
	 		     echo"<input type='hidden' name='nombre_ligne_ue_idp' value=$i >";
	echo"</table>";	
	} 	
	...
	{	
	echo "<table border=1>";
	echo "<th colspan=5 align=center>Voeux ipid</th>";
	echo "</tr><tr>";
	echo "<th></th><th>Code cours</th><th>Intitule</th><th>Crédits ECTS</th>";
	echo"<th>  </th>";	
	 $bgcolor='bgcolor=lightgreen';
	...
	$coursjetons[$uecode]='';
		...
		if (	in_array($uecode ,$ueipid))
		{
				...
				echo"   </td><td>" ;
				echo $coursects[$uecode];
				echo"<input type='hidden' name=champ_ue_form_code_ipid".$i." value='".$uecode."' >";
				$temp='champ_ue_form_jetons_ipid'.$i;
				echo affichemenu('Jetons',$temp,$tab_choix_jetons,$coursjetons[$uecode]);
				echo " </td> ";
				echo "</tr>";		 
			$i++;
	 		     ...
	 		     echo"<input type='hidden' name='nombre_ligne_ue_ipid' value=$i >";
	echo"</table>";	
	} 	
	...
	{	
	echo "<table border=1>";
	echo "<th colspan=5 align=center>Voeux sie</th>";
	echo "</tr><tr>";
	echo "<th></th><th>Code cours</th><th>Intitule</th><th>Crédits ECTS</th>";
	echo"<th>  </th>";	
	 $bgcolor='bgcolor=lightgreen';
	...
	$coursjetons[$uecode]='';
		...
		if (	in_array($uecode ,$uesie))
		{
				...
				echo"   </td><td>" ;
				echo $coursects[$uecode];
				echo"<input type='hidden' name=champ_ue_form_code_sie".$i." value='".$uecode."' >";
				$temp='champ_ue_form_jetons_sie'.$i;
				echo affichemenu('Jetons',$temp,$tab_choix_jetons,$coursjetons[$uecode]);
				echo " </td> ";
				echo "</tr>";		 
			$i++;
	 		     ...
	 		     echo"<input type='hidden' name='nombre_ligne_ue_sie' value=$i >";
	echo"</table>";	
	} 
		  	  		 ...
		  	  		 echo "</tr><tr>";
		echo "<table border=1>";
	echo "<th colspan=5 align=center>Voeux tronc commun</th>";
	echo "</tr><tr>";
	echo "<th></th><th>Code cours</th><th>Intitule</th><th>Crédits ECTS</th>";
	echo"<th> Jetons </th>";	
	 $bgcolor='bgcolor=lightgreen';
	...
	$coursjetons[$uecode]='';
		...
		if (	in_array($uecode ,$uecommun))
		{
				echo"   <tr bgcolor='$bgcolor' ><td>" ;   
				//echo  "<a href='".'http://genie-industriel.grenoble-inp.fr/'.$uecode.'/0/fiche___cours/'."' target=\"_blank\">".'fiche détaillée'."</a>";
				  if(array_key_exists ($uecode ,$fiche_code_ksup ))
				  {				
				if($url_ksup_monobloc==''){echo  "<a href=".$url_ksup_prefixe.$fiche_code_ksup[$uecode].$url_ksup_suffixe." >"."plus d'infos"."</a>";}else{echo  "<a href=".$url_ksup_monobloc." >"."plus d'infos"."</a>";}
				}
				...
				echo"   </td><td>" ;
				echo $coursects[$uecode];
				echo"<input type='hidden' name=champ_ue_form_code_commun".$i." value='".$uecode."' >";
				$temp='champ_ue_form_jetons_commun'.$i;
				echo affichemenu('',$temp,$tab_choix_jetons,$coursjetons[$uecode]);
				echo " </td> ";
				echo "</tr>";		 
			$i++;
		}	
	 }
	 		     echo"<input type='hidden' name='nombre_ligne_ue_commun' value=$i >";
	echo"</table>";	
	echo "</tr><tr>";	
	echo"   </td>" ; 
...
//$orderby="order by $table.date_creation";
  ...
  // on s occupe d'abord des eleves du gpe cible qui ont fait un voeu
   ...
   $query="SELECT  etudiants.Nom ,etudiants.`Prénom 1` ,etudiants.`Code etu` as codetu,etudiants.`Lib dac` as provenance , annuaire.`Mail effectif`,annuaire.`Uid` ,voeux_eleves. * , departements.dep_libelle, groupes.libelle,etudiants_scol.annee
FROM ligne_groupe
...
WHERE libelle = '".$gpecible16."' and (v_id_sondage='".$idsondage16."'  )
";  
      $query.="  ".$orderby;
 //echo $query;
   $result = mysql_query($query,$connexion ); 
...
echo "<a href=ues_voeux_s5.php?voeuxS5idCampagne=".$idsondage16."> Modifier la liste des UEs<br><br></a>";
...
//on initialise  $csv_output
 $csv_output="";
 $csv_output .="code_etu".";"."nom".";"."prenom".";"."mail".";"."groupe".";"."commentaire".";";
 // il faut la liste des ues
 foreach ($uetotal_s5 as $une_ue)
 {
  $csv_output .=$une_ue." (".$courslibel[$une_ue].");";
 }
$csv_output .= "\n";
...
/* 				$csv_output .= nettoiecsv($rep_voeu_parcours1); */
   ...
   // pour l'export excel on recupere tous ses voeux
   	 ...
   	 $query2 = "SELECT distinct `".$table_cours16."`.*,ligne_voeux_ues5.* FROM `".$table_cours16."` 
	 left outer join ligne_voeux_ues5 on ligne_voeux_ues5.ligvs5_code_ue=`".$table_cours16."`.code 
	 where (ligne_voeux_ues5.ligvs5_login = '".$universite->Uid."' and ligvs5_code_idsondage='".$idsondage16."' ) 
	 order by LIBELLE_COURT " ;
	        $result2 = mysql_query($query2,$connexion ); 
			$uechoisies=array();
...
FROM `voeux_eleves`
WHERE `v_id_sondage`='".$idsondage16."'  )
AND libelle  LIKE '".$gpecible16."'
 ORDER BY Nom";
//echo $query;
 $result = mysql_query($query,$connexion ); 
$nombre= mysql_num_rows($result);
