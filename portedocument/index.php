...
$casOn=0;
$texte_table='portedocuments';
// pour afficher dans l'interface le nom des entités de chaque table
$texte_entite='document';
$texte_entite2='étudiant';
$texte_entite3='';
$table="portedocuments";
$cleprimaire='id_portedocument';
$autoincrement='id_portedocument';
$cleetrangere2='codeEtu';
...
$liste_champs_obligatoires=array('libelle');
$liste_champs_lecture_seule_ajout=array();
$liste_champs_lecture_seule_modif=array('codeEtu','modifpar','date_modif','timeline','loginCreateur');
...
$liste_champs_lecture_seule_ou_invisibles_affectes_en_ajout=array('loginCreateur','nomprenomCreateur','emailCreateur');
$liste_champs_invisibles_ajout=array('modifpar','date_modif','timeline','loginCreateur','nomprenomCreateur','emailCreateur');
$liste_champs_invisibles_modif=array('modifpar','date_modif','id_portedocument','loginCreateur');
//----------------------------------------------
...
$getinfovariablevaleur['annee']='codeEtu';
...
$liste_champs_tableau=array( 'Nom', 'Prénom 1','annee','libelle','timeline','nomprenomCreateur');
$liste_champs_filtre=array('Nom','annee');
...
$liste_libelles_tableau=array('Prénom 1'=>'Prénom','loginCreateur'=>'Auteur','annee'=>'Filière');
...
$liste_libelles_champ=array('annee'=>'Filière');
...
$liste_tailles_champ=array('libelle'=>20,'nomprenomCreateur'=>30,'emailCreateur'=>40,'timeline'=>20);
...
//$ajout_sql=",year(date_debut) as year ";
//$ajout_sql=",CASE WHEN month(date_debut) < 9 THEN concat(year(date_debut)-1,'-',year(date_debut))
//ELSE 
...
$login_autorises=array_merge($scol_user_liste,$power_user_liste);
...
$login_autorises_clone=array('patouilm', 'jouffral');
...
$login_autorises_details=array_merge($scol_user_liste,$power_user_liste);
...
//TinyMCE
echo  "   <!-- Infos diverses -->";
// echo  "   <script src=\"js/tiny_mce/tiny_mce.js\"></script>";
echo  "  <script src=\"../js/tinymce/tinymce.min.js\"></script>";
// echo  "   <script src=\"js/config_tinyMCE.js\"></script>";
?>
<script>
  tinymce.init({
    selector: '#texte',
	language: 'fr_FR',
	plugins : "jbimages,link,advlist,textcolor,colorpicker,code,pagebreak,table,save,insertdatetime,preview,media,searchreplace,print,contextmenu,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,template",
    menubar: 'false',
	toolbar: 'undo redo | cut copy paste | bold italic underline strikethrough fontsizeselect forecolor backcolor | alignleft aligncenter alignright | bullist numlist | indent outdent | link unlink | jbimages | code',
	link_context_toolbar: 'true' ,
	fontsize_formats: '8pt 10pt 12pt 14pt 18pt 24pt 36pt 38pt 42pt 45pt 58pt' ,
	color_picker_callback: function(callback, value) {callback('#0099FF'); },
	height : '250px',
	width : '800px',
  });
  </script>
...
/* if ((isset($_GET['clone']) and $_GET['clone'] !='' ))$_POST['clone']=$_GET['clone'];
// pour garder clone apres un form non lancé par un lien 
if ((isset($_POST['clone']) and $_POST['clone'] !='' ))$_GET['clone']=$_POST['clone'];
if ((isset($_GET['code_etud_rech']) and $_GET['code_etud_rech'] !='' ))$_POST['code_etud_rech']=$_GET['code_etud_rech'];
// pour garder code_etud_rech apres un form non lancé par un lien 
if ((isset($_POST['code_etud_rech']) and $_POST['code_etud_rech'] !='' ))$_GET['code_etud_rech']=$_POST['code_etud_rech']; */


/* if (isset($_POST['clone']) and $_POST['clone']!='')
	$filtrerech.="clone=".$_POST['clone']."&";
if (isset($_POST['code_etud_rech']) and $_POST['code_etud_rech']!='')
	$filtrerech.="code_etud_rech=".$_POST['code_etud_rech']."&"; */
//+++++++++++++++++++++++++++++++++++++++++++
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//acces depuis fiche.php
if(isset($_GET['code_etud_rech']) and $_GET['code_etud_rech']!='')
$filtre_initial="where codeEtu = '".$_GET['code_etud_rech']."' and ";
else
$filtre_initial="where ";
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//++++++++++++++++++++++++++++++++++++++++++++++++++
	$_POST['loginCreateur'] =$loginConnecte;
	$_POST['nomprenomCreateur'] =$nomloginConnecte;	
	$_POST['emailCreateur'] =$emailConnecte;	
//++++++++++++++++++++++++++++++++++++++++++++++++++	 
...
//2019 $_POST[$ci2]= html_entity_decode($_POST[$ci2], ENT_QUOTES, 'ISO8859-1');
 ...
 //++++++++++++++++++++++++++++++++++++++++++++++++++
 //pour stocker la date de creation
  elseif($ci2=='timeline')
 {
 $sql1pdo.= $ci2.",";
 //$sql2.= "now(),";
...
if ($err=='')
{
	...
	// il faut récupérer l'id de la fiche qui vient d'être créé
	$query5 = "select max(id_portedocument)as maxid from portedocuments ";
	$req = $connexion->query($query5);
				  	$w=$req->fetch(PDO::FETCH_OBJ);
				  $maxid=$w->maxid;	
	
// on s'occupe du fichier joint
...
if($_FILES['docfil']['tmp_name']!='')
		{
			if($_FILES['docfil']['name']!="")
			{
							$extension = strrchr($_FILES['docfil']['name'], '.'); 
							if(!in_array(strtolower($extension), $extensions))
								//Si l'extension n'est pas dans le tableau
				{
					 $erreur = 'ERREUR ! Vous devez uploader un fichier du type autorisé : .docx,.doc,.pdf';
				}

			}
		}
elseif ($_FILES['docfil']['name']!="")					
			{
			...
			{
			$erreur = "ERREUR ! Vous n'avez pas chargé de fichier  ";
			}
if(!isset($erreur)) //S'il n'y a pas d'erreur, on upload
{
    ...
    if(move_uploaded_file($_FILES['docfil']['tmp_name'], $chemin_local_portedocuments . $fichier)) //Si la fonction renvoie TRUE, c'est que ça a fonctionné...
     {
	...
	// maintenant il faut ajouter l'enregistrement à portedocumentsdocuments
	
	
//$query6="INSERT INTO portedocumentsdocuments(doc_libelle,doc_lienDoc,doc_idportedocument,date_modif,modifpar)
//	VALUES('".$_POST['libelle']."','".$fichier."','".$maxid."',now(),'".$_POST[$champ_modifpar]."')";
//	echo $query6;
$query6="INSERT INTO portedocumentsdocuments(doc_libelle,doc_lienDoc,doc_idportedocument,date_modif,modifpar)
	VALUES(:libelle,:fichier,:maxid,now(),:modifpar)";
$tableaupdo6=	array('libelle'=>$_POST['libelle'],'fichier'=>$fichier,'maxid'=>$maxid,'modifpar'=>$_POST[$champ_modifpar]);
	$req6 = $connexion->prepare($query6 );
		  $res6=$req6->execute($tableaupdo6);	   
		  if ($res6){
		$message .= "Fiche document portedocument <b>"." - ";
	   $message .= "</B> ajoutée !<br>";
		  }
			else {
		echo affichealerte("erreur de saisie ");
	  echo "<center>La fiche document portedocument n'est pas enregistrée</b> </center>";
		}  
}
	else
	{
...
if(in_array($loginConnecte,$login_autorises_suppression) or empty($login_autorises_suppression)){
   ...
   $res1=$pdoquery1->execute(array('del' =>$_GET['del'] ));
   	...
   	while ($u =$pdoquery1->fetch(PDO::FETCH_OBJ))
	{
			...
			$pdoquery2 = $connexion->prepare("select *  FROM portedocumentsdocuments  WHERE doc_idportedocument= :del");
		   ...
		   if ($comptedocs)
		   {
			...
			while ($v =$pdoquery2->fetch(PDO::FETCH_OBJ))
			{
				// on s'occupe d'abord du fichier téléchargé
					$nomdoc=$chemin_local_portedocuments.$v->doc_lienDoc;
			...
			$pdoquery3 = $connexion->prepare("DELETE FROM portedocumentsdocuments  WHERE doc_idDoc= :del");
			   $res3=	    $pdoquery3->execute(array('del' =>$v->doc_idDoc ));
			   if($res3){
			   $message .= "Fiche portedocumentsdocument <b>".$v->doc_idDoc;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	$querydoc=("SELECT * FROM portedocumentsdocuments  where  	doc_idportedocument = :mod ");
	...
	if (!$nombreDocs)
	{
		echo "<td><h2>Il n'y a pas de fichier(s) joint(s) </h2><td></tr> ";	
	}
	else
	{
		echo "<td><h2>Il y a  $nombreDocs fichier(s) joint(s) :</h2><td></tr> ";
		$i=0;
		echo "<table border=1>";
		...
		{
			$i++;
	echo "<tr><td><a target='_blank' href='".$chemin_portedocuments.$u->doc_lienDoc."'</a><h2> fichier ".$i." ".$u->doc_libelle."</a></h2></td></tr>";
		}
		echo "<table>";
	}	
echo "</td></tr>";
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//traitement particulier basé sur le nom du champ

	  elseif ($unchamps == 'codeEtu' ){
		  if ($_GET['code_etud_rech']=='')
			...
			echo afficheonlysqlplus($commentaire,$unchamps,$indexlien2,'select * from '.$table2 ." ".$wherecleeetrangere2." ".$ordrecleeetrangere2,$liste_champs_lies_pour_formulaire_ajout2[0],$_GET['code_etud_rech'],$connexion,$liste_champs_lies_pour_formulaire_ajout2[1]);
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
echo "</tr><tr>";
			echo afficheonly("","Joindre un document  <br> ATTENTION format .jpg,.docx,.doc,.pdf  et taille maxi 5 Mo ",'b' ,'h3','',0);
		...
		echo"       </td><tr>  ";
			echo"       <tr> <td> ";
		echo " choisissez le Fichier à importer  et cliquez ensuite sur Ajouter :<br> <input type='file' name='docfil'>";
				 echo "</td></tr><tr>";	
...
if ($nombre>0){
echo"<center> <h1 class='titrePage2'>Portedocuments &#8594; Liste des   ";
echo $nombre;
echo " ".$texte_entite ."(s)";}
else{
echo"<center> <h2> ";
echo " 0 ".$texte_entite." actuellement dans la base";
}
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//il faut le nom prenom de l'étudiant
if (isset($_GET['code_etud_rech']) and  $_GET['code_etud_rech']!='')
{
$ia=getInfosLigneTable('etudiants',$connexion,$_GET['code_etud_rech'],'Code etu');
echo  " pour ".$ia['Nom']." ".$ia['Prénom 1'];
echo"<br><a href='../fiche.php?code=".$_GET['code_etud_rech']."'>revenir à la page de l'étudiant </a>";
...
if(in_array($loginConnecte,$login_autorises_ajout) or empty($login_autorises_ajout)){
echo "<A href=".$URL."?add=1&".$filtrerech." class='abs'> Ajouter un enregistrement </a><br/>";
}

echo"<table class='table1'> ";


echo"<br><p class='msg'>Vous pouvez changer l'ordre de tri initial en cliquant sur les entêtes des colonnes </p></center>";
if(sizeof($liste_champs_filtre)>0){
	echo"<p class='msg'>Vous pouvez filtrer le tableau en sélectionnant une valeur dans le menu filtre </p></center>";
	}
        echo "<table class='table1'>";
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	$req2 = $connexion->query("SELECT * FROM portedocumentsdocuments  where  	doc_idportedocument =".$u->id_portedocument." ");
...
if(in_array($loginConnecte,$login_autorises_suppression) or empty($login_autorises_suppression)){
	...
	//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 	 	echo" </td><td>";	
if ($nombreDocs)			
     echo "<A class='abs' href=documentportedocuments.php?offre=".$$cleprimaire."&".$filtrerech."from=gest >Pièces jointes(".$nombreDocs. ").</A>";
else
	 echo "<A class='abs' href=documentportedocuments.php?offre=".$$cleprimaire."&add=1&".$filtrerech."from=gest >Ajouter Pièces jointes.</A>";
 	 //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
