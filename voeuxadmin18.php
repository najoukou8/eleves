...
<head>
<title>gestion des voeux Elections</title>
...
$where="where etudiants_scol.annee like '".$gpecible18."%' ";
//$nom_univ='';
...
if(in_array($login,$voeux_liste18)){
...
//on remplit 1tableau avec les libelles-code cours
 ...
 //on remplit 1 tableau avec les noms-code etudiants des 1A
...
$query.= "where groupes.libelle_gpe_constitutif like '1A%' order by etudiants.nom";
$resultat = mysql_query($query,$connexion );
		while ($resultsql=mysql_fetch_object($resultat)) {
			$etudiant_code[]= $resultsql->$myetudiantscode_etu;
			$etudiant_nom_prenom[]= $resultsql->$myetudiantsnom." ".$resultsql->$myetudiantsprénom_1;			
			$etudiant_nom_prenom_corresp[$resultsql->$myetudiantscode_etu]= $resultsql->$myetudiantsnom." ".$resultsql->$myetudiantsprénom_1;
			}
$etudiant_code[]='999999';
$etudiant_nom_prenom[]='NC';
$etudiant_nom_prenom_corresp[999999]='NC';
...
$v=mysql_fetch_object($result) ;
 ...
 //on fait une boucle pour remettre en hiden tous les champs  de la table stage
  ...
  //echo"<input type='hidden' name='mod' value=1>";
	...
	echo "<center><h1>$titrevoeux18</h1><br>";
...
//$mail=ask_ldap($v_login_etud,'mail');
    ...
    echo affichechamp('le','date_modif',$v_date_modif,'15',1);
		  	 ...
		  	 echo "</tr><tr>";
echo affichemenuplus2tab ('Délégué','v_voeux1',$etudiant_nom_prenom,$etudiant_code,$v_voeux1);
echo affichemenuplus2tab ('Suppléant','v_voeux2',$etudiant_nom_prenom,$etudiant_code,$v_voeux2);
echo "</tr><tr>";
		  ...
		  // echo "<td>Commentaire</td>";
		  	  		 // echo "</tr><tr>";
					 // if ($v_commentaire==''){
						// echo "<td colspan=3><textarea   name='v_commentaire' rows=6 cols=60>".$question_commentaire18_1." \n \n\n".$question_commentaire18_2."  </textarea></td> ";		
						// }
						// else
						// {
...
echo"<th>Voix délégué</th>";
echo"<th>Voix suppléant</th>";
echo"<th></th>";
...
//$csv_output .= $champs[$i].";";}
 $csv_output .="voix délégué".";"."voix suppléant".";";
$csv_output .= "\n";
...
FROM voeux_eleves
WHERE v_id_sondage='".$idsondage18."' and v_num_etudiant='".$codeetu."'
";
...
while($universite2=mysql_fetch_object($result2)) {
 ...
 //on fait une boucle pour créer les variables issues de la table voeux et on leur affecte le resultat du select
  ...
  echo"   </td><td>" ;
		  if ($v_voeux1!='9999' and $v_voeux1!=''){
 echo  "<b>a voté</b>";}
 else{
 echo 'NC';
 }
   echo"   </td><td>" ;
		  if ($v_voeux1!='9999' and $v_voeux2!=''){
 echo  "<b>a voté</b>";}
 else{
 echo 'NC';
 }  
	  echo"   </td><td>" ;
      echo $v_commentaire ;
	  // il faut calculer combien de voix il a récolté en voeux1
	 $query="SELECT * FROM voeux_eleves " ;
$query.= "where v_voeux1='".$codeetu."'";
$resultat3 = mysql_query($query,$connexion );  
$nombrevoix1= mysql_num_rows($resultat3);
	  echo"   </td><td>" ;
	  echo $nombrevoix1 ;
	        $csv_output .= nettoiecsvplus($nombrevoix1);
	  echo"   </td><td>" ;
	  // il faut calculer combien de voix il a récolté en voeux1
	 $query="SELECT * FROM voeux_eleves " ;
$query.= "where v_voeux2='".$codeetu."'";
$resultat3 = mysql_query($query,$connexion );  
$nombrevoix2= mysql_num_rows($resultat3);	  
	  echo $nombrevoix2 ;
	  $csv_output .= nettoiecsvplus($nombrevoix2);
	  echo"   </td><td>" ;
	     $csv_output .= "\n";
	  	 if($v_id !=''){
