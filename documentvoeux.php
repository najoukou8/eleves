...
$pdo=1;
// CAS php activé nécessite la présence de cas.php  et du rep CAS dans le rep
$casOn=0;
$texte_table='documents joints';
$table="campagnes_documents";
...
$liste_champs_invisibles=array('doc_idDoc');
...
if (!isset($_GET['clone'])) $_GET['clone']='';
   //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
if (!isset($_GET['voeu'])) $_GET['voeu']='';
if (!isset($_POST['voeu'])) $_POST['voeu']='';
...
if ( $_GET['voeu']!='') 
{
	...
	$filtre_initial=" where doc_idVoeu_eleves = '".$_GET['voeu']."' and " ;
	   //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	// pour ajouter l'etudiant connecté aux listes d'autorisation
	$etudOK=0;
	$personnelOK=0;
	...
	// si il existe bien dans la base eleves 
	 if ($nombre!=0){
		 // il faut encore vérifier que son login est bien celui présent dans la fiche de voeu
		$query= "SELECT  v_login_etud FROM voeux_eleves  where v_id= :voeu";
			$preparequery=$connexion->prepare($query);
			$res=$preparequery->execute(array('voeu' =>$_GET['voeu'] ));			
			$u =$preparequery->fetch(PDO::FETCH_OBJ); 
			if($u->v_login_etud==$loginConnecte)
			{
			 $etudOK=1;
			...
			{echo "problème de concordance d`'identifiants $loginConnecte différent de $u->v_login_etud ";
			}
		 }
	 }
	 else
	 { 
		// ce n'est pas un etudiant
		// on teste si le login est un login d'un personnel autorisé dans cette campagne pour le voeu concerné
		$query= "SELECT  ligne_user_voeux_uid FROM voeux_eleves left outer join  ligne_user_voeux on v_id_sondage=ligne_user_voeux_vid  where v_id= :voeu  ";
		$preparequery=$connexion->prepare($query);
		$res=$preparequery->execute(array('voeu' =>$_GET['voeu'] ));
		$gestionnaires=array();		
		while($u =$preparequery->fetch(PDO::FETCH_OBJ))
		{
			$gestionnaires[]=$u->ligne_user_voeux_uid;			
		}	
		//print_r($gestionnaires);
		if (in_array($loginConnecte,$gestionnaires))
		{
			$personnelOK=1;
			 // on l'ajoute dans les autorisations
			 $login_autorises[]=$loginConnecte; 
...
{
	// si on arrive ici  , c'est qu'il n'y avait pas d'id de voeu
	// on ne peut rien à faire car on n'a pas été ajouté à $login_autorises
 //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
		   //$affichetout=0;
		   //exit();
...
if(in_array($loginConnecte,$login_autorises) or empty($login_autorises) ){
	...
	{$affichetout=0;
	  echo affichealerte(" Vous n'êtes pas autorisé à consulter cette page ( $loginConnecte )");
	}
...
if(!isset($erreur)) //S'il n'y a pas d'erreur, on upload
{
    ...
    if(move_uploaded_file($_FILES['docfil']['tmp_name'], $chemin_local_voeux . $fichier)) //Si la fonction renvoie TRUE, c'est que ça a fonctionné...
     {
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	$nomdoc=$chemin_local_voeux.$r->doc_lienDoc;
...
if($_GET['mod']!=''){
 ...
 $$ci2=mysql_Type_Time($u->$ci2);
  ...
  echo"       <table><tr>  ";
    ...
    //+++++++++++++++++++++++++++on n'affiche pas doc_idVoeu_eleves	+++++++++++++++++++++++++++++++++++++++++++++++++++++++
		 elseif ($unchamps== 'doc_idVoeu_eleves' )
			 {
				  echo"<input type='hidden' name='".$unchamps."' value=\"".$$unchamps."\">";
			 }
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++		
 ...
 //on fait une boucle pour créer les variables issues de la table 
  ...
  // pour garder la ref de l'voeu apres avoir validé le form
    echo"<input type='hidden' name='voeu' value=".$_GET['voeu'].">";
...
if ($unchamps != $autoincrement and $unchamps != $champ_date_modif and $unchamps != $champ_modifpar ){
  ...
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 // on met la ref voeu comme valeur par defaut pour le champ code voeu
	 elseif ($unchamps== 'doc_idVoeu_eleves' ){
    // echo affichechamp($commentaire,$unchamps,$_GET['voeu'],'40','1');	
		echo"<input type='hidden' name='".$unchamps."' value='".$_GET['voeu']."'>";	 
	 }	  
...
// on n'affiche pas le auto inc ni date_modif ni modifpar
  ...
  echo "</td></tr><tr><td><input type='Submit' name='bouton_add' value='ajouter'></form>";
    echo    "<form id='annulation' method=post action=$URL> ";
	 	  //on passe tous les arg reçus en get  en hidden sauf mod et add
	 foreach($_GET as $x=>$ci2)	
	  {
...
if(in_array($loginConnecte,$login_autorises_ajout) or empty($login_autorises_ajout)){
echo "<A href=".$URL."?add=1&".$filtrerech." > Ajouter un document </a><br>";
}
//+++++++++++++++++++++++++++++++++++++++++++++++++++
//pour le clonage dans les voeux c'est login_clone (et pas clone )
echo"<br><br><a href=".$pageaccueil.'?'.$filtrerech."login_clone=".$_POST['clone'].">revenir à l'accueil</a>";
//+++++++++++++++++++++++++++++++++++++++++++++++++++
...
if(in_array($loginConnecte,$login_autorises_suppression) or empty($login_autorises_suppression)){
 ...
 echo "<a href='".$chemin_voeux.$doc_lienDoc ."' target=\"_blank\"> Télécharger</A>";	 
