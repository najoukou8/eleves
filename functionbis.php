...
// fonction de traduction mysql -> mysqli
// suite au passage à php 7 
// normalement on n'a pas à modifier autrechose dans les scripts sources
...
function envoimail($destinataire, $objet, $messagem,$sigiadminmail='gi-dev@grenoble-inp.fr',$discret='') {
	// correction 2019 initialisation des paramètres avant la derniere position
...
function envoimailori($destinataire, $objet, $messagem, $discret='') {
	// on enleve les accents ds l'objet
	$objet = stripAccents($objet);
	// on convertit les entités HTML stockées en leurs caractères correspondant.
	$messagem=html_entity_decode($messagem,ENT_QUOTES,'ISO-8859-1');	
...
function isingroup_giusers($uid,$groupe) { 
		// pour savoir si un  uid est un  membre d'un groupe dont le nom est passé en paramètre 
	// on se connecte 
$dsn="gi_users";
$user_sql="lecture_users";
$password='Acdllmap';
