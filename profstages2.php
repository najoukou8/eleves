...
if($_POST['modif']!='') { //---------------- Modif de la fiche- validation--------------------
  ...
  $query = "UPDATE $table SET etape='2',modifpar='".$login."',date_modif=now(),login_tuteur='".$login."',historique='".$_POST['historique']."'";
   ...
   //echo $query;
      ...
      $message = "Fiche numero ".$_POST['code_stage']." modifiée <br>";
		...
		echo affichealerte("erreur de saisie , veuillez noter l'erreur affichée et l'envoyer à marc.patouillard@inpg.fr "). mysql_error($connexion);
...
envoimail('marc.patouillard@inpg.fr',$objet,$messagem); 
...
if($_POST['refus']!='') { //---------------- Modif de la fiche- refus-------------------
...
// pour les PFE
if ($tutouref=='tuteur'){
$messagem .= "La fiche a été effacée il faudra recommencer une  saisie en choisissant un autre $tutouref \n";}
...
//echo affichechamp('lieu soutenance','soutenance_lieu',$soutenance_lieu,40);
 ...
 } //fin du seules les personnes autorisées ont acces 
  ...
  } // fin du numrows pour retrouver le stage
 }
else {
