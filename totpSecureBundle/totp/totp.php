...
if( $giusers->getOtp($_SERVER['PHP_AUTH_USER']) == null && empty($_SESSION['secret']) ) {
 ...
 if (!empty($_POST['submit']) && !empty($_POST['submit']) && $_POST['submit'] == "valider" ) {
            ...
            if ( $giusers->getOtp($_SERVER['PHP_AUTH_USER']) == null ) {
                ...
                $form = $form . "<br><img style='width: 130px' src='/crypto/sbb.png'><hr style='border-top: 4px solid #2b79b5;'><h1 style='font-weight: bold ; color: #055181 ; font-family: Orbitron; font-size: 2.2em'>&#9888; T-OTP Secure for " . $giusers->getNameById($_SERVER['PHP_AUTH_USER']) . " <br>
<span style='color: black;font-family: Orbitron;font-style: italic;font-size: 25 px'></span></h1>";
            ...
            if ( $giusers->getOtp($_SERVER['PHP_AUTH_USER']) == null ) {
                ...
                $form = $form . "<p>Vous avez perdu votre smartphone ? / Vous arrivez pas a vous connecter ? Recevez le code par mail  <a href='/crypto/sendOtpByMail.php'> ICI </a> </p>";
            }
            ...
            $form = $form . "<form class='form' action='/crypto/totp.php/' method='post'>";
            if ( $giusers->getOtp($_SERVER['PHP_AUTH_USER']) == null ) {
                $qr = $tfa->getQRCodeImageAsDataUri('GI/T-OTP('.$_SERVER['PHP_AUTH_USER'].')', $_SESSION['secret'] ) ;
                $form = $form . "<img class='qr' src=$qr /><hr>";
            }

            $form = $form . "<input type='text' value='' name='tfa_code' style='font-size: 40px;font-family: Orbitron ; text-align: center'><span style='font-size: 3em'>&#x1F510;</span><br>";
            $form = $form . "<br><input type='submit' value='valider' name='submit' class='btn btn-primary text-uppercase btn-block' style='font-size: 2em;font-family: Orbitron;padding:2px;background-color: #055181'>            ";
	...
	if ( isset($_SESSION['doubleAuth']) && !empty($_SESSION['doubleAuth']) )  {
		  ...
		  echo "<script>" ; 
				echo "window.location = 'https://web.gi.grenoble-inp.fr/crypto/'; " ; 
          echo "</script>" ; 
