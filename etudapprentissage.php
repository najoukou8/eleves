...
require ("param.php");
$texte_table='Etudiants en apprentissage';
$table="apprentissageetudiants";
$cleprimaire='etu_idEtu';
$autoincrement='etu_idEtu';
$cleetrangere='';
$table2="";
$cleprimaire2='';
...
$champsobligatoires=array('etu_login','etu_nom');
$liste_champs_dates=array();
$liste_champs_tableau=array('etu_login','etu_nom','etu_prenom','etu_mail','etu_tel');
...
$login_autorises=array_merge($ipid_user_liste);
...
foreach($champsSingle as $ci2){
 ...
 $sql1.= $ci2.",";
 $sql2.= "now(),";}
 elseif($ci2=='etu_login')
 {
 // on passe le login en minuscule
  $sql1.= $ci2.",";
 $sql2.= "'".  strtolower($_POST[$ci2])."',";
 }
...
if(in_array($loginConnecte,$login_autorises_suppression) or empty($login_autorises_suppression)){
	//d'abord on vérifie qu'il n'y plus d'offre avec ce mot clé
	   $query = "select * from apprentissagecomment where com_idEtudiant ='".$_GET['del']."'";
	 $result = mysql_query($query,$connexion);
	 if (mysql_num_rows($result) != 0)
		 {
			 echo affichealerte("il y a ". mysql_num_rows($result) ." fiche(s)  avec cet étudiant mot clé il faut d'abord le supprimer dans les commentaires");
		 }
		else
		{
...
if(in_array($loginConnecte,$login_autorises_ajout) or empty($login_autorises_ajout)){
echo "<A href=".$URL."?add=1> Ajouter un étudiant </a><br>";
}
