...
if (!isset($_POST['id_univ_contact'])) $_POST['id_univ_contact']='';
$message='';
$sql1='';
$sql2='';
...
if ($_POST['id_univ_contact']!=''){
$_GET['filtre_id']=$_POST['id_univ_contact'];
}

if ($_GET['filtre_id']!=''){
$where=" where id_univ_contact=".$_GET['filtre_id']." ";
...
if (!isset($_POST[$ci2])) $_POST[$ci2]='';
 ...
 $sql1=substr($sql1,0,strlen($sql1)-1) ;
  ...
  //echo $query;
      $result = mysql_query($query,$connexion);
       if ($result){
    $message = "Fiche <b>".$_POST['id_contact']." - ";
...
if (!isset($_POST[$ci2])) $_POST[$ci2]='';
 ...
 $sql1=substr($sql1,0,strlen($sql1)-1) ;
   ...
   $query .= " WHERE id_contact=".$_POST['id_contact']." ";
 // echo $query;

   $result = mysql_query($query,$connexion);
   if($result){

   $message = "Fiche numero ".$_POST['id_contact']." modifiée <br>";}
...
if($_GET['mod']!=''){
 ...
 $query = "SELECT * FROM $table 
					  where id_contact=".$_GET['mod']." order by nom ";
					  //echo $query;
...
echo"<input type='hidden' name='id_univ_contact' value=\"".stripslashes(($_GET['filtre_id']))."\">"."\n";
echo afficheonly("","Informations sur le contact",'b' ,'h3');
 echo "</tr><tr>";
     echo affichechamp('Nom','nom',$nom,'','');
	  echo affichechamp('Prénom','prenom',$prenom,'','');
    echo affichechamp('Fonction','fonction',$fonction,'','');
 echo "</tr><tr>";
  echo affichechamp('Adresse1','adresse1',$adresse1,'','');
	    echo affichechamp('Code postal','codepostal',$codepostal,8,'');
    echo affichechamp('Ville','ville',$ville,'','');
	 echo "</tr><tr>";
	//echo affichemenusql('Pays','id_pays','id_pays',"SELECT * FROM pays",'libelle_pays',$id_pays);
		echo affichemenusqlplus('Pays','id_pays','id_pays',"SELECT * FROM pays",'libelle_pays',$id_pays,$connexion);
 echo "</tr><tr>";

	    echo affichechamp('Téléphone','tel',$tel,'','');
    echo affichechamp('Fax','fax',$fax,'','');
	 echo "</tr><tr>";
			echo affichechamp('Email','mail',$mail,'','');
	 echo affichechamp('Site Web','siteweb',$siteweb,'','');

	 	 echo affichemenu('Départ ou accueil','cont_dep_acc',$listeaccdep,$cont_dep_acc,'');
	  echo "</tr><tr>";
	echo "<td colspan=2>commentaires<br><textarea name='comm_contact' rows=2 cols=65>".$comm_contact."</textarea></td> ";	
		  echo "</tr><tr>";
 ...
 echo "</tr><tr>";
	    ...
	    echo affichechamp('Téléphone','tel',$tel,'','');
    echo affichechamp('Fax','fax',$fax,'','','1');
	 echo "</tr><tr>";
			echo affichechamp('Email','mail',$mail,'','');
	 echo affichechamp('Site Web','siteweb',$siteweb,'','');
	 	 	 echo affichemenu('Départ ou accueil','cont_dep_acc',$listeaccdep,'départ','');
	  echo "</tr><tr>";
	echo "<td colspan=2>commentaires<br><textarea name='comm_contact' rows=2 cols=65>".$comm_contact."</textarea></td> ";	
...
echo $nombre;
echo" contacts de ".$nom_univ."</h2></center>  <BR>";}
else{
echo"<center> <h2>Il n'existe pas de contacts    ";
echo" pour $nom_univ</h2></center>  <BR>";}

echo "<A href=".$URL."?add=1&filtre_id=$_GET[filtre_id] > Ajouter un contact </a><br>";
...
echo"<br>En cliquant sur le nom du contact ,vous pouvez lui envoyer un mail </center>";
        echo "<BR><BR><table border=1><tr bgcolor=\"#98B5FF\" > ";

		    if   ($_GET['env_orderby']=='nom' && $_GET['env_inverse']<> 1)
{echo"<th><a href=".$URL."?env_orderby=nom&env_inverse=1&filtre_id=$_GET[filtre_id]>nom</a></th> ";}
else
{echo "<th><a href=".$URL."?env_orderby=nom&filtre_id=$_GET[filtre_id]>nom</a></th> ";}

		    if   ($_GET['env_orderby']=='prenom' && $_GET['prenom']<> 1)
{echo"<th><a href=".$URL."?env_orderby=prenom&env_inverse=1&filtre_id=$_GET[filtre_id]>prenom</a></th> ";}
else
{echo "<th><a href=".$URL."?env_orderby=prenom&filtre_id=$_GET[filtre_id]>prenom</a></th> ";}

		    if   ($_GET['env_orderby']=='fonction' && $_GET['env_inverse']<> 1)
{echo"<th><a href=".$URL."?env_orderby=fonction&env_inverse=1&filtre_id=$_GET[filtre_id]>fonction</a></th> ";}
else
{echo "<th><a href=".$URL."?env_orderby=fonction&filtre_id=$_GET[filtre_id]>fonction</a></th> ";}
 ...
 //on fait une boucle pour créer les variables issues de la table stage
   ...
   $csv_output .= "\n";
	 ...
	 //$modifpar=odbc_result($result,"modifpar") ;
      ...
      echo $fax ;
	  	  	  echo"   </td><td>" ;
     echo " <A href=".$URL."?del=".$id_contact."&filtre_id=$_GET[filtre_id] onclick=\"return confirm('Etes vous sûr de vouloir supprimer ce contact ?')\">";
     echo "sup</A> - ";
     echo "<A href=". $URL."?mod=".$id_contact."&filtre_id=$_GET[filtre_id]>détails</A>";
     echo"        </td> </tr>";
       }
