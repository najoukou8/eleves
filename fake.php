...
<html>
<head>
<title>Accueil Base Elèves 2023/2024</title>
...
if( $_SERVER['SERVER_NAME'] == "localhost" ) {
    echo "<h1 style='margin-top:-11px;font-family :Vertexio; text-align:center ; background-color : #d72f2f ; color : white ; padding : 10px ;text-transform: uppercase; font-size : 50px; font-weight:bold'><img id='loader' src='./icons/ajax3.gif' style='width:32px;'/> *Base des élèves $annee_accueil </h1> ";
}else{
	    //echo "<h1 style='margin-top:-11px;font-family :Vertexio; text-align:center ; background-color : #f57d11 ; color : white ; padding : 10px ;text-transform: uppercase; font-size : 50px; font-weight:bold'><img id='loader' src='./icons/ajax3.gif' style='width:32px;'/> *Base des élèves $ecole $annee_accueil </h1> ";
        echo "<h1 style='margin-top:-11px;font-family :Vertexio; text-align:center ; background-color : #2b79b5 ; color : white ; padding : 5px ;text-transform: uppercase; font-size : 50px; font-weight:bold'><img id='loader' src='./icons/ajax3.gif' style='width:32px;'/> Base élèves $annee_accueil </h1>";
}
    ...
    $count1=mysql_query("SELECT COUNT(*) AS count1 FROM `etudiants` WHERE `Année Univ` = '2023'",$connexion );
    $count1_ = 0 ;
...
<div class="wrapper">
    ...
    <?php
    if ($count_prov > 0  ) { ?>
        <div class="box b" id='websiteavis1' style='background-color:#17a2b8 ; color : white; animation: blink 0.8s;animation-iteration-count:infinite;border:4px #0b899d solid;' > &#9203; Chargement ... </div>
        <?php
    }
    ?>
  ...
  <div class="box a" id='websiteavis5' style='background-color:#4a545d ; color : white' > &#9203; Chargement ...</div>
  ...
  <div class="box b" id='websiteavis3' style='background-color:#dc3545 ; color : white' > &#9203; Chargement ... </div>
    ...
    <div class="container">
        <div class="marquee" style='background-color: #82fa9d !important;color:black'>
            <div>
                <b style="background-color: GREEN  ; padding: 4px ; opacity: 0.8 ; border-radius: 4px ;"> <span style="color: white ; font-size: 16px;">&#9888; </span> * INFORMATION : </b>13/11/23 10h15 > Double authentification est désactivée pour ceux qui ont déja valider leurs QR code .    
            </div>
        </div>
    </div>
...
?>
    ...
    <div class="doughnut" style=" min-width: 400px !important; float: right ; margin-top: 10px">
        <div class="doughnutChartContainer"><iframe class="chartjs-hidden-iframe" style="width: 100%; display: block; border: 0px none; height: 0px; margin: 0px; position: absolute; inset: 0px;"></iframe>
            <canvas id="myChart" style="width: 400px; height:  100%;"></canvas>
        </div>
    </div>
...
echo "<br><i class='fa-solid fa-file-zipper'></i> Archive Base élèves <a href='https://web.gi.grenoble-inp.fr/eleves2022' style='background-color: #ffc107;
color: red;
padding: 2px;
border-radius: 4px;'> 2022/2023 </a><a href='https://web.gi.grenoble-inp.fr/eleves2021' style='background-color: #ffc107;
color: red; margin-left:2px;
padding: 2px;
border-radius: 4px;'> 2021/2022 </a>";
...
echo" <select name='code_groupe_peda'  onchange='nom_recherche.value=null;monform.submit()' onkeypress='monform.submit()' style='width:90.68%;height:30px;margin-top:2px;' >  ";
...
echo "<center><h2 class='titrePage2'>".RemoveAccents2($gpetitre)."</h2></center>";
...
<script>
    ...
    <script>
            ...
            window.onload=function(){
                ...
                var promisedDeliveryChart = new Chart(document.getElementById('myChart'), {
                    ...
                    options: {
                        ...
                        title: {
                            display: true,
                            text: 'REPARTIONS DES '+percent2 + ' ETUDIANT(S)'
                        },
						scales: {
            xAxes: [{
                ticks: {
        ...
        Chart.defaults.global.defaultFontSize = 10.7 ;
