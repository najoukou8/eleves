...
<head>
    ...
    //$filtre='';
	//$loginConnecte = 'foukan' ; 
    $login_autorises_clone=array('administrateur','foukan','lemairpi');
    ...
    // si on a le droit
    if (in_array($loginConnecte,$login_autorises_clone,'foukan') ) {
    ...
    $urlrefens="https://refens.grenoble-inp.fr/gi/".($annee_courante-2)."/recherche/suggestions?format=html&nomComposante=gi&texteRecherche=";
...
//$filtre="fil=".urlencode($_GET['fil'])."&lignea0=".urlencode($_GET['lignea0']);
            ...
            echo "<h2>  coût pour ".$translitfilieres[$_GET['fil']]." en  heures eq TD  pour année " .($annee_courante-2)."-". ($annee_courante-1)."</h2>";

            echo "<table id=t1>";
            echo "<tr>";
            echo "<td  width=50%>";
// on récupère les heures payées ( dans hel)  sans correspondance refens
            $query0="select sum(`Nb heure eff. EqTD`) as nbre_heures_hors_refens from hel  left join effectifs on `Matiere`=`code apogee` where `code apogee` is null";
...
//echo $query;
            ...
            while ($r=mysql_fetch_array($result))
            {
                ...
                else $temp =round($r[$_GET['fil']] /$r['inscrits'],4);
...
// on garde tout
            ...
            foreach($resultats as $ligne)
            {
                ...
                if (1 OR $ligne['totalheffective']!=0 or (isset($_GET['lignea0']) and $_GET['lignea0']==1 ))
                    //	if(1)
                {
					...
					pour donner accès > pour afficher tout 
...
//		echo "<a href=?detail=".$ligne['code'].">".$ligne['nombre']."</a>";
                        ...
                        if ((int)$ligne['coutAP']!=(int)$ligne['totalheffective'] and $_GET['fil']=='tout')
                        {
                            ...
                            echo "</td><td align=right>";
                        if (1 OR in_array('pilotage',$groupes) )
...
// on double quote dans le $_GET['detail']
            ...
            {
                $query="select *  from hel  left join effectifs on `Matiere`=`code apogee` where `code apogee` is null and `Nb heure eff. EqTD` !='' ";
            }
            else
            {
                $query="SELECT * FROM `hel` WHERE `Matiere` = '".$_GET['detail']."'";
            }
