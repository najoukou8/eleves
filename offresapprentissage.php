...
//$cleetrangere='off_id_entreprise';
$cleetrangere2='off_id_entreprise';
...
$wherecleeetrangere2=" where entreprises.entreprise_apprent = 'oui' ";
$table2="entreprises";
$indexlien2='code';
$cleetrangere3='';
$ordrecleeetrangere3='';
...
$champ_date_modif='off_date_modif';
$champ_modifpar='off_modifpar';

$liste_champs_lies2=array('nom','ville');
...
$liste_champs_lies_pour_formulaire_ajout2=array('nom','');
//$liste_champs_lies_pour_formulaire_ajout 2 maxi , si un seul mettre une chaine vide dans le 2eme element du tableau
...
$liste_champs_invisibles=array('off_idOffre','off_Entreprise','off_ville','off_adresse1','off_adresse2','off_cp','off_nomResponsableAdmin','off_prenomResponsableAdmin','off_mailResponsableAdmin');
$liste_champs_dates=array('off_dateSaisie');
$liste_champs_heures=array();
$liste_champs_tableau=array('nom','off_lieuMission','off_pourvue','off_refOffre');
$liste_champs_filtre=array('off_pourvue','nom','off_lieuMission','off_refOffre');
...
$liste_libelles_tableau=array('nom'=>'nom entreprise');
...
$liste_libelles_champ=array('off_id_entreprise'=>'entreprise');
...
$liste_valeur_defaut=array('off_pourvue'=>'non');
//pour  l'ordre d'affichage
$liste_ordre_champ=array('off_pourvue'=>6);
...
$login_autorises=array('anceychr','rogesj','administrateur','cataldic');
...
$login_autorises_clone=$ipid_poweruser_liste;
// pour pouvoir  ajouter
$login_autorises_ajout=$ipid_poweruser_liste;
...
$login_autorises_details=array('anceychr','rogesj','administrateur','cataldic');
...
$req0 = $connexion->query("select * from apprentissageetudiants" );
 $nombre0=$req0->rowCount();
//$result = mysql_query("select * from apprentissageetudiants");	
if ( $nombre0 > 0) {
while ($row = $req0->fetch(PDO::FETCH_OBJ)) {	
...
//$login_autorises=array_merge($login_autorises,$login_etudiants);
$login_autorises=array_merge($ipid_user_liste,$login_etudiants);
$login_autorises_details=array_merge($ipid_user_liste,$login_etudiants);
...
{
//// on récupère le login du connecté 
if($_SERVER['AUTH_USER']!=''){
	list($domaine,$loginConnecte)=explode("\\",$_SERVER['AUTH_USER']);
	$loginConnecte=strtolower($loginConnecte);}
	else
	{ $domaine='';$loginConnecte=''; }
}
...
echo "<h1 class='titrePage2' style='text-align: center'>  Térrains d'apprentissage </h1>" ;
...
if(in_array($loginConnecte,$login_autorises_suppression) or empty($login_autorises_suppression)){
   ...
   $res1=$pdoquery1->execute(array('del' =>$_GET['del'] ));
   	...
   	while ($u =$pdoquery1->fetch(PDO::FETCH_OBJ))
	{
			...
			$pdoquery2 = $connexion->prepare("select *  FROM apprentissagedocuments  WHERE doc_idOffre= :del");
		   $res2=$pdoquery2->execute(array('del' =>$_GET['del'] ));
		   $comptedocs=$pdoquery2->rowCount();
		   if ($comptedocs)
		   {
			echo affichealerte(" il y a $comptedocs documents associé à cette fiche  , on le(s) supprime aussi ")  ;
		   }
			while ($v =$pdoquery2->fetch(PDO::FETCH_OBJ))
			{
				// on s'occupe d'abord du fichier téléchargé
					$nomdoc=$chemin_local_apprentissage.$v->doc_lienDoc;
			...
			$pdoquery3 = $connexion->prepare("DELETE FROM apprentissagedocuments  WHERE doc_idDoc= :del");
			   $res3=	    $pdoquery3->execute(array('del' =>$v->doc_idDoc ));
			   if($res3){
			   $message .= "Fiche document <b>".$v->doc_idDoc;
			   $message .= "</b> supprimée <br>!";
			   }
			
			}
			 // il faut aussi supprimer les commentaires liés
   
				$pdoquery = $connexion->prepare("SELECT * FROM apprentissagecomment  WHERE com_IdOffre= :del");
			   $res=	    $pdoquery->execute(array('del' =>$_GET['del'] ));
			   if($res){
					$nombre=$pdoquery->rowCount();
			   $message .= "$nombre commentaires liés à la fiche  <b>".$_GET['del'];
			   $message .= "</b> supprimé(s) <br>!";
			   }   
...
// 2018
 ...
 //on passe tous les arg reçus en get  en hidden pour le form du bouton annuler
 ...
 //on fait une boucle pour créer les variables issues de la table 
  ...
  echo    "<form method=post action=$URL> ";
  	 	  ...
  	 	  //on passe tous les arg reçus en get  en hidden sauf add
...
if ($nombre>0){
echo"<center> <h2 style='margin-top: -15px ; background-color : #2b79b5 ; color : white ; padding : 8px ;text-transform: uppercase; font-size : 24px; font-weight:bold'>Liste des   ";
echo $nombre;
echo " ".$texte_table ."</H2>";}
else{
echo"<center> <h2> ";
...
if(in_array($loginConnecte,$login_autorises_ajout) or empty($login_autorises_ajout)){
echo "<A class='abs1' href=".$URL."?add=1&".$filtrerech." > Ajouter une offre </a>";
}
echo"<br><a class='abs' href=".$pageaccueil.">Revenir à l'Accueil</a>";
//2018

if(in_array($loginConnecte,$login_autorises_admin) or empty($login_autorises_admin)){
	echo"<br><a href=entreprises_apprent.php>Gestion des entreprises</a>";
echo"<br>";
...
//2018
	...
	$req2 = $connexion->query("SELECT * FROM apprentissagecomment  where  	com_idOffre =".$u->off_idOffre." ");	
	$nombreCommentaires=$req2->rowCount();	
	
	$req2 = $connexion->query("SELECT * FROM apprentissagedocuments  where  	doc_idOffre =".$u->off_idOffre." ");
...
if((in_array($loginConnecte,$login_autorises_suppression) or empty($login_autorises_suppression))){
     echo " <A class='abs2' href=".$URL."?del=".$$cleprimaire."&".$filtrerech." onclick=\"return confirm('Etes vous sûr de vouloir supprimer cet enregistrement ainsi que les fichiers et les commentaires correspondant ?')\">";
     echo "Sup</A> - ";
	 }
	 if (in_array($loginConnecte,$login_autorises_modif) or empty($login_autorises_modif) ){
     echo "<A class='abs1' href=". $URL."?mod=".$$cleprimaire."&".$filtrerech." >Mod</A>";
	  echo" </td><td>";
	 echo "<A class='abs' href=commentapprentissage.php?offre=".$$cleprimaire." >Commentaires(".$nombreCommentaires. ") -</A>";	 
	echo" </td><td>";	  
     echo "<A class='abs' href=documentapprentissage.php?offre=".$$cleprimaire." >Docs(".$nombreDocs. ").</A>";	
     ...
     echo "<A class='abs' href=". $URL."?mod=".$$cleprimaire."&".$filtrerech." >Détails</A>";	 
	  echo" </td><td>";
	 echo "<A class='abs' href=commentapprentissage.php?offre=".$$cleprimaire."&".$filtrerech."  >Commentaires(".$nombreCommentaires. ") -</A>";	 
	echo" </td><td>";	  
     echo "<A class='abs' href=documentapprentissage.php?offre=".$$cleprimaire."&".$filtrerech."  >Docs(".$nombreDocs. ").</A>";	
