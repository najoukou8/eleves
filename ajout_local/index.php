...
$texteintro= "<h2>gestion des comptes locaux</h2>";
...
$liste_champs_filtre=array('user_nom');
...
$filtre_initial="where user_password != '' and ";
...
$login_autorises_clone=array();
// pour pouvoir  ajouter
$login_autorises_ajout=array('patouilm','administrateur','dehemchn','anceychr','cataldic');
// pour pouvoir  supprimer
$login_autorises_suppression=array('patouilm','administrateur','dehemchn','anceychr','cataldic');
// pour pouvoir  modifier
$login_autorises_modif=array('administrateur','patouilm','dehemchn','anceychr','cataldic');
...
$login_autorises_export=array('patouilm','administrateur','dehemchn','anceychr','cataldic');
...
$_POST['user_password']=random_string(10);
$_POST['user_password_hash']= '{SHA}' . base64_encode(sha1( $_POST['user_password'], TRUE ));
// attention aux espaces !
$login_calcule='_'.nettoie(substr($_POST['user_nom'],0,7).substr($_POST['user_prenom'],0,1));
// on vérifie si il existe déja
$query1="select * from people where user_login = ?";
$req = $connexion->prepare($query1 );
$res=$req->execute(array($login_calcule));
$result=$req->fetchAll();
if (count($result) >0 )
	$err= "Le login $login_calcule existe déjà ";

$_POST['user_login']=$login_calcule;
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++		 
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++	
// il faut aussi ajouter ce compte dans le groupe local_only

$querypdo='insert into lignes_groupes(groupe_id,people_id) values(3,?)';
		  $req = $connexion->prepare($querypdo );
		  $res=$req->execute(array($_POST['user_login']));
		   if ($res){
		$message .= "Fiche <b>"." - ";
	   $message .= "</B> ajoutée au groupe localonly !<br>";
		   }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++	
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++	
// il faut aussi envoyer un mail avec les infos
 echo " on envoie un mail aux gestionnaires<br>";
 
  $messagem.="création d'un compte provisoire pour ".$_POST['user_nom']." ".$_POST['user_prenom'] ." par  ".$nomloginConnecte ."  \n";	
  $messagem.="nom  : ".$_POST['user_nom'] ."  \n";	
  $messagem.="prenom  : ".$_POST['user_prenom'] ."  \n";
  $messagem.="email  : ".$_POST['user_email'] ."  \n";  
  $messagem.="login  : ".$_POST['user_login'] ."  \n";	  
  $messagem.="mot de passe  : ".$_POST['user_password'] ."  \n";	
  $messagem.="date limite de validité  : ".mysql_DateTime($_POST['user_date_limite']) ."  \n";  
			  // On prepare l'email : on initialise les variables
		$objet = "création d'un compte provisoire pour ".$_POST['user_nom']." ".$_POST['user_prenom'] ;
				// On envoi l’email à la gestionnaire 

		if ($emailConnecte !=''){			   
		envoimailtest($emailConnecte,htmlspecialchars_decode($objet),htmlspecialchars_decode($messagem),'',1);
		envoimail($emailConnecte,htmlspecialchars_decode($objet),htmlspecialchars_decode($messagem));		
		}
