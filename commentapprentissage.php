...
$casOn=1;
$texte_table='Commentaires';
$table="apprentissagecomment";
$cleprimaire='com_idCom';
$autoincrement='com_idCom';
$cleetrangere2='com_idEtudiant';
...
$wherecleeetrangere2="";
$table2="apprentissageetudiants";;
$indexlien2='etu_login';
$cleetrangere3='com_idOffre';
$ordrecleeetrangere3='';
// restriction dans le  select en saisie / modification - peut être vide
$wherecleeetrangere3="";
$table3="apprentissageoffres";
$indexlien3='off_idOffre';
// pour la gestion des champs modifpar et date_modif
//laissez vide si pas géré
$champ_date_modif='com_date_modif';
$champ_modifpar='com_modifpar';

$liste_champs_lies2=array('etu_nom','etu_prenom');
$liste_champs_lies3=array('off_lieuMission','off_descriptif');
//$liste_champs_lies_pour_formulaire_ajout 2 maxi , si un seul mettre une chaine vide dans le 2eme element du tableau
$liste_champs_lies_pour_formulaire_ajout2=array('etu_nom','etu_prenom');
//$liste_champs_lies_pour_formulaire_ajout 2 maxi , si un seul mettre une chaine vide dans le 2eme element du tableau
$liste_champs_lies_pour_formulaire_ajout3=array('off_refOffre','');
// au moins un (cléprimaire)
$liste_champs_obligatoires=array();
$liste_champs_lecture_seule=array();
$liste_champs_invisibles=array('com_dateCreation');
$liste_champs_dates=array('com_dateEntretien');
$liste_champs_heures=array();
$liste_champs_tableau=array('com_texte','com_idOffre','etu_nom','com_selection');
...
$login_autorises_ajout=array_merge($ipid_user_liste,$login_etudiants);
...
if ( $_GET['offre']=='') {$filtre_initial=" where ";}
else {$filtre_initial=" where com_idOffre = '".$_GET['offre']."' and " ;}
...
//2018
//Pour récupérer les checkboxs et mettre le tout dans commentaire
$_POST['com_texte']='';
if (isset($_POST['commentaires']))
{
foreach($_POST['commentaires'] as $ci4){
	$_POST['com_texte'] .=$ci4 ."/";	
}
}
else
...
// pour autre commentaire

$_POST['com_texte'] .=$_POST['autre_commentaire'];
...
if($_GET['mod']!=''){
 ...
 $$ci2=mysql_Type_Time($u->$ci2);
     ...
     echo    "<form method=post action=$URL> ";
	 	  ...
	 	  //on passe tous les arg reçus en get  en hidden sauf mod
...
//2018
			if($unchamps == 'com_idEtudiant' or $unchamps == 'com_idOffre')
				 {
				echo affichechamp($commentaire,$unchamps,$$unchamps,'5','1');	
			 }	
//-----------------			 
...
// 2018
 ...
 //on fait une boucle pour créer les variables issues de la table 
  ...
  echo"       <table><tr>  ";
	 ...
	 // on n'affiche pas le auto inc ni date_modif ni modifpar et on leur affecte la bonne valeur en hidden
...
//2018	
	 ...
	 if ($unchamps== 'com_idOffre' ){
     //echo affichechamp($commentaire,$unchamps,$_GET['offre'],'40','1');	
	 echo"<input type='hidden' name='".$unchamps."' value='".$_GET['offre']."'>";	 
	 }	  
	 elseif ($unchamps== 'com_idEtudiant' ){
    // echo affichechamp($commentaire,$unchamps,$loginConnecte,'40','1');	
	 echo"<input type='hidden' name='".$unchamps."' value='".$loginConnecte."'>";	 
	 }		 
	 // si on a une table liée
	 //2018 on n'affiche plus 
	//  elseif ($unchamps == $cleetrangere2 ){
				  ...
				  elseif (in_array($unchamps,$liste_champs_lies3) ){
					...
					//]]>
					  ...
					  //2018
					   ...
					   elseif($unchamps == 'com_selection')
		 {
		 echo afficheradio($commentaire,$unchamps,$listeouinon,$$unchamps,'non','');	
		}	
		 elseif($unchamps == 'com_texte')
		 {
			 //2018
			 echo"<td>";
			 // on affiche des checkboxs
			 foreach ($comm_offres_app as $unchoix)
			 {
				 echo  "<input type='checkbox' name='commentaires[]' value='".$unchoix."' />".$unchoix."<br>";
			 }
			  echo"</td>";
			  echo "</tr><tr>";
			 echo affichechamp('autre commentaire','autre_commentaire','','80','0');	

			 	 echo"</td>";
		//echo affichemenu ('commentaire',$unchamps,$comm_offres_app,$$unchamps);	
		}	
...
if((in_array($loginConnecte,$login_autorises_ajout) or empty($login_autorises_ajout)) and $_GET['offre'] !=''){
echo "<A href=".$URL."?add=1&offre=".$_GET['offre']."> Ajouter un commentaire pour l'offre numéro " .$_GET['offre']." </a><br>";
}
if(in_array($loginConnecte,$login_autorises_modif)){
echo "<A href=".$URL."> Afficher tous les commentaires </a><br>";
}
